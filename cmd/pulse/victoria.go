...
func victoriaEnabled() bool {
	value, ok := os.LookupEnv("PULSE_DEV_VICTORIA")
	if !ok {
		return true
	}
...
func victoriaDownloadEnabled() bool {
	value, ok := os.LookupEnv("PULSE_DEV_VICTORIA_DOWNLOAD")
	if !ok {
		return true
	}
	return !isFalseEnv(value)
...
func victoriaRootDir(appRoot string) string {
	if value := strings.TrimSpace(os.Getenv("PULSE_DEV_VICTORIA_DIR")); value != "" {
		return value
	}
	return filepath.Join(appRoot, ".pulse", "victoria")
}
	...
	return []victoriaComponentSpec{
		{
			...
			Version:          envOrDefault("PULSE_VICTORIA_METRICS_VERSION", "v1.141.0"),
			DefaultPort:      intEnvOrDefault("PULSE_VICTORIA_METRICS_PORT", 8428),
			EndpointPath:     "/opentelemetry/v1/metrics",
			URLPath:          "/vmui",
			StorageDir:       "metrics-data",
			EnvPrefix:        "PULSE_VICTORIA_METRICS",
			OTELVar:          "OTEL_EXPORTER_OTLP_METRICS_ENDPOINT",
			PulseURLVar:      "PULSE_VICTORIA_METRICS_URL",
			PulseEndpointVar: "PULSE_VICTORIA_METRICS_ENDPOINT",
		},
		{
			Name:             "logs",
			...
			Version:          envOrDefault("PULSE_VICTORIA_LOGS_VERSION", "v1.50.0"),
			DefaultPort:      intEnvOrDefault("PULSE_VICTORIA_LOGS_PORT", 9428),
			EndpointPath:     "/insert/opentelemetry/v1/logs",
			URLPath:          "/select/vmui",
			StorageDir:       "logs-data",
			EnvPrefix:        "PULSE_VICTORIA_LOGS",
			OTELVar:          "OTEL_EXPORTER_OTLP_LOGS_ENDPOINT",
			PulseURLVar:      "PULSE_VICTORIA_LOGS_URL",
			PulseEndpointVar: "PULSE_VICTORIA_LOGS_ENDPOINT",
		},
		{
			Name:             "traces",
			...
			Version:          envOrDefault("PULSE_VICTORIA_TRACES_VERSION", "v0.8.1"),
			DefaultPort:      intEnvOrDefault("PULSE_VICTORIA_TRACES_PORT", 10428),
			EndpointPath:     "/insert/opentelemetry/v1/traces",
			URLPath:          "/select/vmui",
			StorageDir:       "traces-data",
			EnvPrefix:        "PULSE_VICTORIA_TRACES",
			OTELVar:          "OTEL_EXPORTER_OTLP_TRACES_ENDPOINT",
			PulseURLVar:      "PULSE_VICTORIA_TRACES_URL",
			PulseEndpointVar: "PULSE_VICTORIA_TRACES_ENDPOINT",
		},
	}
}
...
func (s *victoriaStack) Env() []string {
	...
	env := []string{"PULSE_DEV_OBSERVABILITY_BACKEND=victoria"}
	for _, component := range s.components {
		env = append(env,
			component.spec.OTELVar+"="+component.endpointURL,
			component.spec.PulseURLVar+"="+component.baseURL,
			component.spec.PulseEndpointVar+"="+component.endpointURL,
		)
	}
	return env
}
