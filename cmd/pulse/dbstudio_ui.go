...
func prepareDBStudioUIDir(ctx context.Context, console *runConsole) (string, error) {
	if dir := strings.TrimSpace(os.Getenv("PULSE_DEV_DBSTUDIO_UI_DIR")); dir != "" {
		return dir, nil
	}
	...
	return prepareUIDir(ctx, console, uiBuildSpec{
		envVar:       "PULSE_DEV_DBSTUDIO_UI_DIR",
		root:         filepath.Join(app.RepoRoot(), "dbstudio"),
		installTitle: "Installing Pulse DB Studio UI packages",
		buildTitle:   "Building Pulse DB Studio UI",
...
func (s *dbStudioUIServer) Start(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("pulse db studio UI failed to listen on %s: %w", dbStudioUIAddr, err)
	}
	go func() {
		<-ctx.Done()
...
func (s *dbStudioUIServer) handle(w http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		_, _ = io.WriteString(w, `<html><body><p>Pulse DB Studio UI build is not available. Run <code>bun run build</code> inside <code>dbstudio/</code>.</p></body></html>`)
		return
	}
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	w.Header().Set("Cache-Control", "no-store")
