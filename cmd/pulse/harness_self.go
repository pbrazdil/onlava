...
func runPulseHarnessSelf(ctx context.Context, stdout io.Writer, args []string) error {
	...
	resp := harnessSelfResponse{
		SchemaVersion: "pulse.harness.self.v1",
		OK:            true,
		GeneratedAt:   time.Now().UTC().Format(time.RFC3339Nano),
		Repo: harnessSelfRepo{
			Root:       repoRoot,
			ModulePath: "pulse.dev",
	...
	resp.Steps = append(resp.Steps,
		...
		runHarnessExecStep(ctx, repoRoot, "go tests", []string{"go", "test", "./cmd/pulse", "./internal/devdash", "./runtime"}),
		...
		runHarnessFreshnessStep("dashboard ui fresh", filepath.Join(repoRoot, "ui"), dashboardUIBuildStale, "Run `bun run build` inside `ui/`, then rerun `pulse harness self --json`."),
		...
		runHarnessFreshnessStep("db studio ui fresh", filepath.Join(repoRoot, "dbstudio"), dbStudioUIBuildStale, "Run `bun run build` inside `dbstudio/`, then rerun `pulse harness self --json`."),
		runHarnessExecStep(ctx, repoRoot, "install pulse binary", []string{"go", "install", "./cmd/pulse"}),
		runHarnessPulseBinaryStep(repoRoot),
	)
	...
	if opts.Write {
		resp.Wrote = filepath.Join(repoRoot, ".pulse", "harness", "self-latest.json")
	}
	...
	if opts.JSON {
		...
		if !resp.OK {
			return &silentCLIError{err: fmt.Errorf("pulse harness self failed")}
		}
		return nil
	}
	...
	if !resp.OK {
		return fmt.Errorf("pulse harness self failed")
	}
	return nil
}
...
func runHarnessInspectDocsStep(repoRoot string) harnessStep {
	...
	err := runPulseInspect([]string{"docs", "--repo-root", repoRoot, "--json"}, &out)
	step := harnessStep{
		Name:       "inspect docs",
		Command:    []string{"pulse", "inspect", "docs", "--repo-root", repoRoot, "--json"},
		OK:         err == nil,
	...
	if err != nil {
		...
		step.Diagnostics = []checkDiagnostic{{
			...
			SuggestedAction: "Run `pulse inspect docs --json`, fix the reported docs issue, then rerun `pulse harness self --json`.",
		}}
		return step
	}
	...
	if err := json.Unmarshal(out.Bytes(), &payload); err != nil {
		...
		step.Diagnostics = []checkDiagnostic{{
			...
			Severity:        "error",
			Message:         step.Error,
			SuggestedAction: "Fix `pulse inspect docs --json` output so it conforms to pulse.inspect.docs.v1.",
		}}
		return step
	}
	...
	if payload.Summary.MissingCount > 0 || payload.Summary.StaleCount > 0 {
		...
		step.Diagnostics = []checkDiagnostic{{
			...
			SuggestedAction: "Run `pulse inspect docs --json`, update docs/knowledge.json or the referenced docs, then rerun `pulse harness self --json`.",
		}}
	}
	return step
}
...
func discoverPulseRepoRoot(start string) (string, error) {
	if start == "" {
		if cwd, err := os.Getwd(); err == nil {
			if root, ok := findPulseRepoRoot(cwd); ok {
				return root, nil
			}
		}
	...
	if found, ok := findPulseRepoRoot(root); ok {
		return found, nil
	}
	return "", fmt.Errorf("no Pulse repo root found from %s", root)
}

func findPulseRepoRoot(start string) (string, bool) {
	dir := filepath.Clean(start)
	for {
		data, err := os.ReadFile(filepath.Join(dir, "go.mod"))
		if err == nil {
			text := string(data)
			if strings.HasPrefix(text, "module pulse.dev\n") || strings.Contains(text, "\nmodule pulse.dev\n") {
				return dir, true
			}
		}
...
func runHarnessFreshnessStep(name, root string, staleFn func(string) (bool, error), suggestion string) harnessStep {
	...
	step := harnessStep{
		Name:       name,
		Command:    []string{"pulse", "harness", "self", "internal:freshness-check", root},
		DurationMS: time.Since(started).Milliseconds(),
...
func runHarnessPulseBinaryStep(repoRoot string) harnessStep {
	started := time.Now()
	step := harnessStep{
		Name:       "pulse binary fresh",
		Command:    []string{"go", "install", "./cmd/pulse"},
		DurationMS: 0,
	}
	pulsePath, err := exec.LookPath("pulse")
	if err != nil {
		step.OK = false
		step.DurationMS = time.Since(started).Milliseconds()
		step.Error = "pulse not found in PATH"
		step.Diagnostics = []checkDiagnostic{{
			Stage:           step.Name,
			Severity:        "error",
			Message:         step.Error,
			SuggestedAction: "Run `go install ./cmd/pulse` from the Pulse repo and ensure your Go bin directory is in PATH.",
		}}
		return step
	}
	binaryInfo, err := os.Stat(pulsePath)
	if err != nil {
		step.OK = false
		step.DurationMS = time.Since(started).Milliseconds()
	...
	if ok && binaryInfo.ModTime().Before(latest) {
		...
		step.Diagnostics = []checkDiagnostic{{
			Stage:           step.Name,
			Severity:        "error",
			Message:         "installed pulse binary is older than repo sources",
			SuggestedAction: "Run `go install ./cmd/pulse` from the Pulse repo.",
		}}
	} else {
		step.OK = true
	}
...
func runHarnessKnowledgeStep(repoRoot string) harnessStep {
	...
	step := harnessStep{
		...
		Command: []string{"pulse", "harness", "self", "internal:knowledge-check", repoRoot},
		Summary: map[string]any{
	...
	for _, item := range knowledge.Schemas {
		...
		if !json.Valid(data) {
			diagnostics = append(diagnostics, checkDiagnostic{
				...
				SuggestedAction: "Fix the JSON syntax, then rerun `pulse harness self --json`.",
			})
		}
	}
		...
		for _, raw := range markdownLinkTargets(string(data)) {
			...
			if _, err := os.Stat(targetPath); err != nil {
				diagnostics = append(diagnostics, checkDiagnostic{
					...
					SuggestedAction: "Fix or remove the broken local link, then rerun `pulse harness self --json`.",
				})
			}
		}
	}
...
func validateExecPlanContract(repoRoot string) ([]checkDiagnostic, map[string]any) {
	...
	if err != nil {
		diagnostics = append(diagnostics, checkDiagnostic{
			...
			Message:         err.Error(),
			SuggestedAction: "Create PLANS.md with the Pulse ExecPlan contract.",
		})
	} else {
...
func validateExecPlanSections(repoRoot, relPath, text string, standard bool) []checkDiagnostic {
	var diagnostics []checkDiagnostic
	if standard && !strings.Contains(text, "Pulse Execution Plans") {
		diagnostics = append(diagnostics, execPlanDiagnostic(repoRoot, relPath, 1, "PLANS.md does not identify itself as the Pulse ExecPlan standard", "Keep PLANS.md as the canonical Pulse ExecPlan contract."))
	}
...
func buildHarnessSelfKnowledge(repoRoot string) harnessKnowledge {
	...
	schemas := []string{
		"docs/schemas/pulse.admin.result.v1.schema.json",
		"docs/schemas/pulse.app.v1.schema.json",
		"docs/schemas/pulse.build.latest.v1.schema.json",
		"docs/schemas/pulse.docs.index.v1.schema.json",
		"docs/schemas/pulse.harness.self.v1.schema.json",
		"docs/schemas/pulse.harness.result.v1.schema.json",
		"docs/schemas/pulse.check.result.v1.schema.json",
		"docs/schemas/pulse.gen.manifest.v1.schema.json",
		"docs/schemas/pulse.inspect.app.v1.schema.json",
		"docs/schemas/pulse.inspect.build.v1.schema.json",
		"docs/schemas/pulse.inspect.docs.v1.schema.json",
		"docs/schemas/pulse.inspect.endpoints.v1.schema.json",
		"docs/schemas/pulse.inspect.metrics.v1.schema.json",
		"docs/schemas/pulse.inspect.paths.v1.schema.json",
		"docs/schemas/pulse.inspect.routes.v1.schema.json",
		"docs/schemas/pulse.inspect.services.v1.schema.json",
...
func buildHarnessSelfArtifacts(repoRoot string, selfWillExist bool) []harnessArtifact {
	artifacts := []harnessArtifact{
		{Name: "self-harness", Path: ".pulse/harness/self-latest.json", SchemaVersion: "pulse.harness.self.v1"},
...
func writeHarnessSelfText(w io.Writer, resp harnessSelfResponse) error {
	...
	if _, err := fmt.Fprintf(w, "pulse: self harness %s\n", status); err != nil {
		return err
	}
	for _, step := range resp.Steps {
		marker := "ok"
...
func installSuggestion(binary string) string {
	switch binary {
	case "bun":
		return "Install Bun or ensure it is available in PATH, then rerun `pulse harness self --json`."
	case "go":
		return "Install Go or ensure it is available in PATH, then rerun `pulse harness self --json`."
	default:
		return "Install `" + binary + "` or ensure it is available in PATH, then rerun `pulse harness self --json`."
	}
}

func rerunSuggestion(command []string, dir string) string {
	return "Run `" + strings.Join(command, " ") + "` in `" + dir + "`, fix the failure, then rerun `pulse harness self --json`."
}
