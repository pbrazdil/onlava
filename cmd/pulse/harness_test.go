...
func TestParseHarnessSelfArgs(t *testing.T) {
	opts, err := parseHarnessSelfArgs([]string{"--repo-root", "/tmp/pulse", "--json", "--write"})
	if err != nil {
		t.Fatalf("parseHarnessSelfArgs returned error: %v", err)
	}
	if opts.RepoRoot != "/tmp/pulse" {
...
func TestFindPulseRepoRoot(t *testing.T) {
	root := t.TempDir()
	nested := filepath.Join(root, "cmd", "pulse")
	if err := os.MkdirAll(nested, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(root, "go.mod"), []byte("module pulse.dev\n\ngo 1.26.0\n"), 0o644); err != nil {
		t.Fatal(err)
	}

	got, ok := findPulseRepoRoot(nested)
	if !ok {
...
func TestRunHarnessKnowledgeStepSuccess(t *testing.T) {
	...
	writeTestAppFile(t, root, "docs/harness-engineering.md", "[schema](schemas/pulse.harness.self.v1.schema.json)\n")
...
func TestRunPulseHarnessJSONSuccessWritesLatest(t *testing.T) {
	root := t.TempDir()
	...
	if err := runPulseHarness(context.Background(), &out, []string{"--app-root", root, "--json", "--write"}); err != nil {
		t.Fatalf("runPulseHarness returned error: %v\n%s", err, out.String())
	}
	...
	if payload.SchemaVersion != "pulse.harness.result.v1" || !payload.OK {
...
func TestRunPulseHarnessJSONFailureIncludesNextAction(t *testing.T) {
	root := t.TempDir()
	cacheRoot := filepath.Join(t.TempDir(), "cache")
	t.Setenv("PULSE_DEV_CACHE_DIR", cacheRoot)
	...
	var out bytes.Buffer
	err := runPulseHarness(context.Background(), &out, []string{"--app-root", root, "--json"})
...
func writeHarnessTestApp(t *testing.T, root, name, body string) {
	t.Helper()
	writeTestAppFile(t, root, "pulse.app", `{"name":"`+name+`","id":"`+name+`-id"}`)
...
func writeHarnessSelfRepo(t *testing.T, schema string) string {
	t.Helper()
	root := t.TempDir()
	writeTestAppFile(t, root, "go.mod", "module pulse.dev\n\ngo 1.26.0\n")
  ...
  "schema_version": "pulse.docs.index.v1",
  "generated_at": "2026-04-27T00:00:00Z",
  "owner_default": "Pulse maintainers",
  "freshness_policy": {
  ...
  "documents": [
    ...
    },
    {
      ...
      "summary": "Contract.",
      "tags": ["contract"],
      "schema_refs": ["docs/schemas/pulse.docs.index.v1.schema.json"]
    }
  ],
  "plans": {
...
func validExecPlanStandardForTest() string {
	...
	b.WriteString("# Pulse Execution Plans\n\n")
