...
func TestRunPulseInspectRequiresJSON(t *testing.T) {
	err := runPulseInspect([]string{"app"}, &bytes.Buffer{})
	if err == nil || err.Error() != "pulse inspect currently requires --json" {
		t.Fatalf("runPulseInspect() error = %v", err)
	}
}

func TestRunPulseInspectOutputsStableJSON(t *testing.T) {
	root := t.TempDir()
	cacheRoot := filepath.Join(t.TempDir(), "cache")
	t.Setenv("PULSE_DEV_CACHE_DIR", cacheRoot)
	writeTestAppFile(t, root, "pulse.app", `{"name":"inspectapp","id":"inspect-id"}`)
...
`)
	...
	t.Run("app", func(t *testing.T) {
		...
		if err := runPulseInspect([]string{"app", "--json"}, &out); err != nil {
			t.Fatalf("runPulseInspect(app) error = %v", err)
		}
		var payload struct {
		...
		if payload.SchemaVersion != "pulse.inspect.app.v1" {
	...
	t.Run("services", func(t *testing.T) {
		var out bytes.Buffer
		if err := runPulseInspect([]string{"services", "--json"}, &out); err != nil {
			t.Fatalf("runPulseInspect(services) error = %v", err)
		}
		var payload struct {
			SchemaVersion string `json:"schema_version"`
			Services      []struct {
		...
		if payload.SchemaVersion != "pulse.inspect.services.v1" {
			t.Fatalf("schema_version = %q", payload.SchemaVersion)
		}
	...
	t.Run("routes", func(t *testing.T) {
		var out bytes.Buffer
		if err := runPulseInspect([]string{"routes", "--json"}, &out); err != nil {
			t.Fatalf("runPulseInspect(routes) error = %v", err)
		}
		var payload struct {
			SchemaVersion string `json:"schema_version"`
			Routes        []struct {
		...
		if payload.SchemaVersion != "pulse.inspect.routes.v1" {
			t.Fatalf("schema_version = %q", payload.SchemaVersion)
		}
	...
	t.Run("build", func(t *testing.T) {
		var out bytes.Buffer
		if err := runPulseInspect([]string{"build", "--json"}, &out); err != nil {
			t.Fatalf("runPulseInspect(build) error = %v", err)
		}
		var payload struct {
			SchemaVersion string `json:"schema_version"`
			Build         struct {
		...
		if payload.SchemaVersion != "pulse.inspect.build.v1" {
			t.Fatalf("schema_version = %q", payload.SchemaVersion)
		}
	...
	t.Run("paths", func(t *testing.T) {
		var out bytes.Buffer
		if err := runPulseInspect([]string{"paths", "--json"}, &out); err != nil {
			t.Fatalf("runPulseInspect(paths) error = %v", err)
		}
		var payload struct {
			SchemaVersion string `json:"schema_version"`
			Paths         struct {
		...
		if payload.SchemaVersion != "pulse.inspect.paths.v1" {
			t.Fatalf("schema_version = %q", payload.SchemaVersion)
		}
...
func TestRunPulseInspectExcludesUnrelatedPackages(t *testing.T) {
	root := t.TempDir()
	writeTestAppFile(t, root, "pulse.app", `{"name":"inspectapp","id":"inspect-id"}`)
	writeTestAppFile(t, root, "go.mod", "module example.com/inspectapp\n\ngo 1.26.0\n\nrequire pulse.dev v0.0.0\n\nreplace pulse.dev => "+repoRootForTest(t)+"\n")
	writeTestAppFile(t, root, "users/api.go", `package users
...
func TestRunPulseInspectBuildUsesLatestManifest(t *testing.T) {
	root := t.TempDir()
	writeTestAppFile(t, root, "pulse.app", `{"name":"manifestapp","id":"manifest-id"}`)
	writeTestAppFile(t, root, ".pulse/build/latest.json", `{
  "schema_version": "pulse.build.latest.v1",
  "app": {
    "name": "manifestapp",
    "id": "manifest-id",
    "root": "`+root+`",
  ...
  "build": {
    ...
    "binary_path": "/tmp/custom-workspace/pulse-app",
    "workspace_exists": true,
    "binary_exists": true,
    "build_state_path": "/tmp/custom-workspace/.pulse-build-state.json",
...
func TestRunPulseInspectUsesGeneratedArtifacts(t *testing.T) {
	root := t.TempDir()
	writeTestAppFile(t, root, "pulse.app", `{"name":"genapp","id":"gen-id"}`)
	writeTestAppFile(t, root, ".pulse/gen/app.json", `{
  ...
  "auth_handler": {
	...
	writeTestAppFile(t, root, ".pulse/gen/routes.json", `{
  "schema_version": "pulse.inspect.routes.v1",
  "app": {
    "name": "genapp",
    "id": "gen-id",
    "root": "`+root+`",
