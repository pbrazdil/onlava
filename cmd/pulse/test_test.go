...
func TestPulseTestRunsGoTestInGeneratedWorkspace(t *testing.T) {
	root := t.TempDir()
	writeTestAppFile(t, root, "pulse.app", `{"name":"testapp"}`)
	...
	writeTestAppFile(t, root, "svc/api_test.go", "package svc\n\nimport (\n\t\"testing\"\n\n\tpulse \"pulse.dev\"\n)\n\nfunc TestPulseMetaUsesTestEnv(t *testing.T) {\n\tif pulse.Meta().Environment.Type != pulse.EnvTest {\n\t\tt.Fatalf(\"env type = %q, want %q\", pulse.Meta().Environment.Type, pulse.EnvTest)\n\t}\n}\n")

	restore := chdirForTest(t, filepath.Join(root, "svc"))
	defer restore()

	if err := runPulseTest(context.Background(), []string{"-run", "TestPulseMetaUsesTestEnv"}); err != nil {
		t.Fatalf("runPulseTest returned error: %v", err)
	}
}
...
func TestPulseTestPassesThroughGoTestFlags(t *testing.T) {
	if testing.Short() && runtime.GOOS == "windows" {
		t.Skip("slow process test on windows")
	}
	root := t.TempDir()
	writeTestAppFile(t, root, "pulse.app", `{"name":"flagapp"}`)
	...
	if err := runPulseTest(context.Background(), []string{"./svc", "-run", "TestOne"}); err != nil {
		t.Fatalf("runPulseTest returned error: %v", err)
	}
}
