...
func traceEventsToOTLP(events []*devdash.TraceEvent) []any {
	...
	for _, event := range events {
		...
		attrs := []any{otlpAttr("pulse.event_id", event.EventID)}
		if len(event.Event) > 0 {
			attrs = append(attrs, otlpAttr("pulse.event", event.Event))
		}
...
func buildOTLPTraceProto(summary *devdash.TraceSummary, events []*devdash.TraceEvent) []byte {
	...
	for _, event := range events {
		...
		evt = append(evt, protoMessage(3, protoKeyValue("pulse.event_id", event.EventID))...)
		if len(event.Event) > 0 {
			evt = append(evt, protoMessage(3, protoKeyValue("pulse.event", event.Event))...)
		}
...
func buildOTLPMetricProto(summary *devdash.TraceSummary) []byte {
	...
	metric := protoString(1, "pulse.request.duration")
	metric = append(metric, protoString(2, "Pulse request duration")...)
...
func resourceAttributePairs(appID string) []otlpAttribute {
	return []otlpAttribute{
		...
		{Key: "telemetry.sdk.name", Value: "pulse"},
...
func traceSummaryAttributePairs(summary *devdash.TraceSummary) []otlpAttribute {
	attrs := []otlpAttribute{
		{Key: "pulse.app_id", Value: summary.AppID},
		{Key: "pulse.trace.type", Value: summary.Type},
		{Key: "pulse.is_root", Value: summary.IsRoot},
		{Key: "pulse.is_error", Value: summary.IsError},
		{Key: "pulse.service", Value: summary.ServiceName},
	}
	if summary.EndpointName != nil {
		attrs = append(attrs, otlpAttribute{Key: "pulse.endpoint", Value: *summary.EndpointName})
	}
	if summary.TopicName != nil {
		attrs = append(attrs, otlpAttribute{Key: "pulse.pubsub.topic", Value: *summary.TopicName})
	}
	if summary.SubscriptionName != nil {
		attrs = append(attrs, otlpAttribute{Key: "pulse.pubsub.subscription", Value: *summary.SubscriptionName})
	}
	if summary.MessageID != nil {
		attrs = append(attrs, otlpAttribute{Key: "pulse.pubsub.message_id", Value: *summary.MessageID})
	}
	return attrs
}

func logAttributePairs(event *devdash.LogEvent) []otlpAttribute {
	attrs := []otlpAttribute{{Key: "pulse.app_id", Value: event.AppID}}
	for key, value := range event.Attrs {
		attrs = append(attrs, otlpAttribute{Key: "pulse.log." + key, Value: value})
	}
	return attrs
}
...
func protoInstrumentationScope() []byte {
	out := protoString(1, "pulse.dev")
	out = append(out, protoString(2, pulseVersion)...)
	return out
}
