...
func checkCommand(args []string) error {
	return runPulseCheck(context.Background(), os.Stdout, args)
}

func runPulseCheck(ctx context.Context, stdout io.Writer, args []string) error {
	...
	if opts.JSON {
		return writeCheckJSON(stdout, checkResponse{
			SchemaVersion: "pulse.check.result.v1",
			OK:            true,
			App:           appInfo,
			Diagnostics:   []checkDiagnostic{},
		})
	}
	_, _ = fmt.Fprintln(stdout, "pulse: check ok")
	return nil
}
...
func suggestedActionForDiagnostic(stage, message string) string {
	...
	case strings.Contains(message, "undefined:"):
		return "Define the missing symbol or add the required import, then rerun `pulse check --json`."
	case strings.Contains(message, "no matching files found"):
		return "Ensure the referenced file exists at build time and rerun `pulse check --json`."
	case strings.Contains(message, "updates to go.mod needed"):
		return "Run `go mod tidy` in the app and rerun `pulse check --json`."
	case stage == "parse":
		return "Fix the source or Pulse directive error, then rerun `pulse check --json`."
	case stage == "prepare":
		return "Fix the generated workspace or dependency setup issue, then rerun `pulse check --json`."
	default:
		return "Fix the compile error, then rerun `pulse check --json`."
	}
}
