...
func TestRunPulseInspectDocs(t *testing.T) {
	root := writeHarnessSelfRepo(t, `{"$schema":"https://json-schema.org/draft/2020-12/schema","type":"object"}`)

	var out bytes.Buffer
	if err := runPulseInspect([]string{"docs", "--repo-root", root, "--json"}, &out); err != nil {
	...
	if payload.Repo.Root != root || payload.Repo.ModulePath != "pulse.dev" {
