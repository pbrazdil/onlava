...
func TestVictoriaEnabledCanBeDisabled(t *testing.T) {
	for _, value := range []string{"0", "false", "no", "off"} {
		t.Run(value, func(t *testing.T) {
			t.Setenv("PULSE_DEV_VICTORIA", value)
			if victoriaEnabled() {
...
func TestResolveVictoriaBinaryPrefersExplicitEnv(t *testing.T) {
	...
	t.Setenv("PULSE_VICTORIA_TRACES_BIN", binary)
...
func TestVictoriaStackEnv(t *testing.T) {
	...
	env := stack.Env()
	if !containsString(env, "PULSE_DEV_OBSERVABILITY_BACKEND=victoria") {
				...
				map[string]any{
					...
					"spans": []any{
						map[string]any{
							...
							"processID":     "p1",
							"tags": []any{
								map[string]any{"key": "pulse.service", "type": "string", "value": "svc"},
								map[string]any{"key": "pulse.endpoint", "type": "string", "value": "Hello"},
								map[string]any{"key": "pulse.is_error", "type": "bool", "value": false},
							},
						},
					},
				},
