...
func harnessCommand(args []string) error {
	return runPulseHarness(context.Background(), os.Stdout, args)
}

func runPulseHarness(ctx context.Context, stdout io.Writer, args []string) error {
	if len(args) > 0 && args[0] == "self" {
		return runPulseHarnessSelf(ctx, stdout, args[1:])
	}
	...
	resp := harnessResponse{
		SchemaVersion: "pulse.harness.result.v1",
		OK:            true,
	...
	if opts.Write {
		resp.Wrote = filepath.Join(appRoot, ".pulse", "harness", "latest.json")
	}
	...
	if opts.JSON {
		...
		if !resp.OK {
			return &silentCLIError{err: fmt.Errorf("pulse harness failed")}
		}
		return nil
	}
	...
	if !resp.OK {
		return fmt.Errorf("pulse harness failed")
	}
	return nil
}
...
func runHarnessCheck(ctx context.Context, appRoot string) (harnessStep, inspect.AppRef) {
	...
	err := runPulseCheck(ctx, &out, []string{"--app-root", appRoot, "--json"})
	step := harnessStep{
		Name:       "check",
		Command:    []string{"pulse", "check", "--app-root", appRoot, "--json"},
		OK:         err == nil,
...
func runHarnessInspect(subject, appRoot string) harnessStep {
	started := time.Now()
	var out bytes.Buffer
	err := runPulseInspect([]string{subject, "--app-root", appRoot, "--json"}, &out)
	step := harnessStep{
		Name:       "inspect " + subject,
		Command:    []string{"pulse", "inspect", subject, "--app-root", appRoot, "--json"},
		OK:         err == nil,
		DurationMS: time.Since(started).Milliseconds(),
	}
	if err != nil {
...
func buildHarnessArtifacts(appRoot string, harnessWillExist bool) []harnessArtifact {
	artifacts := []harnessArtifact{
		{Name: "app", Path: ".pulse/gen/app.json", SchemaVersion: "pulse.inspect.app.v1"},
		{Name: "routes", Path: ".pulse/gen/routes.json", SchemaVersion: "pulse.inspect.routes.v1"},
		{Name: "services", Path: ".pulse/gen/services.json", SchemaVersion: "pulse.inspect.services.v1"},
		{Name: "endpoints", Path: ".pulse/gen/endpoints.json", SchemaVersion: "pulse.inspect.endpoints.v1"},
		{Name: "wire", Path: ".pulse/gen/wire/capabilities.json", SchemaVersion: "pulse.wire.capabilities.v1"},
		{Name: "gen-manifest", Path: ".pulse/gen/manifest.json", SchemaVersion: "pulse.gen.manifest.v1"},
		{Name: "latest-build", Path: ".pulse/build/latest.json", SchemaVersion: "pulse.build.latest.v1"},
		{Name: "latest-harness", Path: ".pulse/harness/latest.json", SchemaVersion: "pulse.harness.result.v1"},
	}
...
func writeHarnessText(w io.Writer, resp harnessResponse) error {
	...
	if _, err := fmt.Fprintf(w, "pulse: harness %s for %s\n", status, resp.App.Name); err != nil {
		return err
	}
