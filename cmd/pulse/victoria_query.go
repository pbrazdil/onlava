...
func traceSummaryFromVictoriaSpan(appID string, trace victoriaJaegerTrace, span victoriaJaegerSpan) *devdash.TraceSummary {
	...
	serviceName := stringTag(tags, "pulse.service")
	if serviceName == "" {
		serviceName = process.ServiceName
	}
	endpointName := optionalVictoriaString(firstNonEmpty(stringTag(tags, "pulse.endpoint"), endpointFromOperation(span.OperationName)))
	summary := &devdash.TraceSummary{
		AppID:         appID,
		TraceID:       traceID,
		SpanID:        span.SpanID,
		Type:          firstNonEmpty(stringTag(tags, "pulse.trace.type"), "REQUEST"),
		IsRoot:        len(span.References) == 0,
		IsError:       boolTag(tags, "pulse.is_error") || boolTag(tags, "error") || stringTag(tags, "otel.status_code") == "ERROR",
	...
	if topic := stringTag(tags, "pulse.pubsub.topic"); topic != "" {
		summary.TopicName = &topic
	}
	if subscription := stringTag(tags, "pulse.pubsub.subscription"); subscription != "" {
		summary.SubscriptionName = &subscription
	}
	if messageID := stringTag(tags, "pulse.pubsub.message_id"); messageID != "" {
