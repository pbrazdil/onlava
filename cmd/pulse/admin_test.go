...
func TestRunPulseAdminRequiresJSON(t *testing.T) {
	err := runPulseAdmin(context.Background(), []string{"traces", "clear"}, &bytes.Buffer{})
	if err == nil || err.Error() != "pulse admin currently requires --json" {
		t.Fatalf("runPulseAdmin() error = %v", err)
	}
}

func TestRunPulseAdminClearTraces(t *testing.T) {
	root := t.TempDir()
	cacheRoot := t.TempDir()
	t.Setenv("PULSE_DEV_CACHE_DIR", cacheRoot)
	writeTestAppFile(t, root, "pulse.app", `{"name":"adminapp","id":"admin-id"}`)
	...
	if err := runPulseAdmin(context.Background(), []string{"traces", "clear", "--json"}, &out); err != nil {
		t.Fatalf("runPulseAdmin(traces clear) error = %v", err)
	}
	var payload struct {
	...
	if payload.SchemaVersion != "pulse.admin.result.v1" || !payload.OK || payload.Command != "pulse admin traces clear" {
...
func TestRunPulseAdminClearPubSubViaDashboardRPC(t *testing.T) {
	root := t.TempDir()
	writeTestAppFile(t, root, "pulse.app", `{"name":"adminapp","id":"admin-id"}`)
	...
	t.Setenv("PULSE_DEV_DASHBOARD_ADDR", strings.TrimPrefix(server.URL, "http://"))

	restore := chdirForTest(t, root)
	defer restore()

	var out bytes.Buffer
	if err := runPulseAdmin(context.Background(), []string{"pubsub", "clear", "--json"}, &out); err != nil {
		t.Fatalf("runPulseAdmin(pubsub clear) error = %v", err)
	}
	var payload struct {
		OK   bool `json:"ok"`
		Data struct {
