...
set -eu
trap 'echo interrupted > "$PULSE_TEST_INTERRUPTED"; kill "$child" 2>/dev/null || true; wait "$child" 2>/dev/null || true; exit 0' INT TERM
sleep 30 &
child=$!
...
`, map[string]string{
		...
		"PULSE_TEST_INTERRUPTED": interruptPath,
...
func TestSecondCtrlCUsesDefaultSignalBehavior(t *testing.T) {
	if os.Getenv("PULSE_TEST_SECOND_CTRL_C_HELPER") == "1" {
	...
	cmd.Env = append(os.Environ(),
		"PULSE_TEST_SECOND_CTRL_C_HELPER=1",
		"PULSE_TEST_READY="+readyPath,
		"PULSE_TEST_FIRST="+firstPath,
	)
	cmd.Stdin = nil
...
func runSecondCtrlCHelper() {
	...
	_ = os.WriteFile(os.Getenv("PULSE_TEST_READY"), []byte("ready"), 0o644)
	go func() {
		<-sigCtx.Done()
		stopSignals()
		cancel()
	...
	<-ctx.Done()
	_ = os.WriteFile(os.Getenv("PULSE_TEST_FIRST"), []byte("first"), 0o644)
	select {}
}
