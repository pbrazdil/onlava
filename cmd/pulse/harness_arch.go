...
var forbiddenSourceImports = map[string]string{
	"github.com/julienschmidt/httprouter": "Pulse uses the standard-library router/runtime routing instead of httprouter.",
	"github.com/spf13/cobra":              "Pulse CLI intentionally stays hand-rolled to avoid framework surface area.",
	"github.com/urfave/cli":               "Pulse CLI intentionally stays hand-rolled to avoid framework surface area.",
	"github.com/fatih/color":              "Pulse terminal styling uses internal/termstyle instead of a color dependency.",
	"github.com/charmbracelet/lipgloss":   "Pulse terminal styling uses internal/termstyle instead of a UI framework dependency.",
}
...
func runHarnessArchitectureStep(repoRoot string) harnessStep {
	...
	step := harnessStep{
		...
		Command: []string{"pulse", "harness", "self", "internal:architecture-check", repoRoot},
	...
	if err != nil {
		diagnostics = append(diagnostics, checkDiagnostic{
			...
			Severity:        "error",
			Message:         err.Error(),
			SuggestedAction: "Fix the source walk error, then rerun `pulse harness self --json`.",
		})
	} else {
...
func checkArchitectureDependencies(repoRoot string, summary *architectureSummary) []checkDiagnostic {
	...
	if err != nil {
		return []checkDiagnostic{{
			...
			Message:         err.Error(),
			SuggestedAction: "Restore go.mod, then rerun `pulse harness self --json`.",
		}}
	}
	...
	if err != nil {
		return []checkDiagnostic{{
			...
			File:            filepath.ToSlash(filepath.Join(repoRoot, "go.mod")),
			Message:         err.Error(),
			SuggestedAction: "Fix go.mod syntax, then rerun `pulse harness self --json`.",
		}}
	}
	var diagnostics []checkDiagnostic
	...
	for _, imp := range file.Imports {
		...
		if reason, ok := forbiddenSourceImports[importPath]; ok {
			diagnostics = append(diagnostics, checkDiagnostic{
				...
				SuggestedAction: "Use the existing Pulse standard-library/internal implementation instead.",
			})
		}
		if importPath == "C" {
			diagnostics = append(diagnostics, checkDiagnostic{
		...
		if importPath == "pulse.dev/cmd/pulse" && !strings.HasPrefix(rel, "cmd/pulse/") {
			diagnostics = append(diagnostics, checkDiagnostic{
				Stage:           "architecture checks",
				Severity:        "error",
				File:            filepath.ToSlash(path),
				Message:         "non-CLI package imports pulse.dev/cmd/pulse",
