...
const (
	inspectTracesSchema  = "pulse.inspect.traces.v1"
	inspectMetricsSchema = "pulse.inspect.metrics.v1"
)
...
func openObservabilityStore(ctx context.Context, appRoot string, cfg appcfg.Config) (*devdash.Store, []string, error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			warnings = append(warnings, "no local observability state found for "+cfg.Name+"; run `pulse run` first")
