...
func adminCommand(args []string) error {
	return runPulseAdmin(context.Background(), args, os.Stdout)
}

func runPulseAdmin(ctx context.Context, args []string, stdout io.Writer) error {
	opts, err := parseAdminArgs(args)
	if err != nil {
		return err
	}
	if !opts.JSON {
		return fmt.Errorf("pulse admin currently requires --json")
	}
	...
	resp := adminResponse{
		SchemaVersion: "pulse.admin.result.v1",
		OK:            true,
		Command:       "pulse admin " + opts.Domain + " " + opts.Action,
		App: adminAppRef{
			Name: cfg.Name,
			Root: appRoot,
		},
...
func parseAdminArgs(args []string) (adminOptions, error) {
	if len(args) < 2 {
		return adminOptions{}, fmt.Errorf("usage: pulse admin traces clear --json [--app-root <path>] | pulse admin pubsub clear --json [--app-root <path>]")
	}
	opts := adminOptions{
		Domain: args[0],
		Action: args[1],
...
func callDashboardRPC(ctx context.Context, method string, params any) (json.RawMessage, error) {
	...
	if err != nil {
		if resp != nil {
			return nil, fmt.Errorf("pulse dashboard RPC unavailable: %s", resp.Status)
		}
		return nil, fmt.Errorf("pulse dashboard RPC unavailable: %w", err)
	}
	defer conn.Close()
