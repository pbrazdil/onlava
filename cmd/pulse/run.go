...
func parseRunArgs(args []string) (runOptions, error) {
	...
	for i := 0; i < len(args); i++ {
		...
		case "--verbose", "-v", "--json", "--dashboard", "--watch", "--db-studio", "--proxy":
			return runOptions{}, fmt.Errorf("%s is a development flag; use `pulse dev`", args[i])
		default:
...
func startHeadlessApp(root string, cfg app.Config, binary, addr string, opts runOptions) error {
	...
	overrides := []string{
		...
		"PULSE_APP_ID=" + cfg.Name,
		"PULSE_APP_ROOT=" + root,
		"PULSE_LOCAL_PROXY=0",
		"PULSE_LOG_FORMAT=" + opts.LogFormat,
		"PULSE_PARENT_MONITOR=1",
		fmt.Sprintf("PULSE_PARENT_MONITOR_PID=%d", os.Getpid()),
	}
	if opts.Env != "" {
		overrides = append(overrides, "PULSE_ENV="+opts.Env, "PULSE_RUNTIME_ENV="+opts.Env)
	}
	...
	if err != nil {
		return fmt.Errorf("pulse app exited: %w", err)
	}
	return nil
}
