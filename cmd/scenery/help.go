...
var helpCommands = []helpCommandEntry{
	...
	},
	{
		...
		Usage:       []string{"scenery harness [--app-root <path>] [--json] [--write] [--with-validation[=<profile>]]", "scenery harness self [--repo-root <path>] [--summary|--json|--json=summary|--json=full] [--write] [--quick|--race|--release]", "scenery harness ui --json [--app-root <path>] [--dashboard-url <url>] [--headed] [--write]"},
		Subcommands: []string{"self", "ui"},
		Flags:       []string{"--app-root <path>", "--repo-root <path>", "--summary", "--json", "--json=summary", "--json=full", "--write", "--quick", "--race", "--release", "--dashboard-url <url>", "--headed", "--with-validation[=<profile>]"},
		JSON:        true,
		Stability:   "stable",
	},
	{
