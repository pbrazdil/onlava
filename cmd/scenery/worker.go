...
func runWorkerTypeScript(opts workerTypeScriptOptions, stdout io.Writer) error {
	...
	if !cfg.Temporal.Enabled {
		return fmt.Errorf("scenery worker typescript requires temporal.enabled=true to run; use --generate-only to generate the worker without starting Temporal")
	}
...
func typeScriptWorkerCommand(runtimeName string) (string, []string, error) {
	...
	if runtimeName == "" {
		...
		return "", nil, fmt.Errorf("scenery worker typescript requires bun or node in PATH")
	}
...
func startTypeScriptWorker(root string, cfg app.Config, info sceneryruntime.TemporalRuntimeInfo, runtimeName string, args []string, taskQueues []string) error {
	...
	if len(taskQueues) > 0 {
		extra = append(extra, "SCENERY_TEMPORAL_TASK_QUEUE="+strings.Join(taskQueues, ","))
	}
	...
	if err != nil {
		return fmt.Errorf("scenery TypeScript worker exited: %w", err)
	}
	return nil
}
...
func startWorkerApp(root string, cfg app.Config, binary string, opts workerOptions) error {
	...
	if len(opts.TaskQueues) > 0 {
		extra = append(extra, "SCENERY_TEMPORAL_TASK_QUEUE="+strings.Join(uniqueWorkerTaskQueues(opts.TaskQueues), ","))
	}
