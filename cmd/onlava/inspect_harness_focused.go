...
func knownHarnessArtifacts() []harnessArtifact {
	return []harnessArtifact{
		...
		{Name: "toolchain", Path: ".onlava/harness/toolchain-latest.json", SchemaVersion: harnessToolchainSchema},
		{Name: "changed-area", Path: ".onlava/harness/changed-area-latest.json", SchemaVersion: harnessChangedAreaSchema},
		{Name: "drift", Path: ".onlava/harness/drift-latest.json", SchemaVersion: harnessDriftSchema},
		{Name: "test-timing", Path: ".onlava/harness/test-timing-latest.json", SchemaVersion: harnessTestTimingSchema},
		{Name: "fixture-matrix", Path: ".onlava/harness/fixture-matrix-latest.json", SchemaVersion: harnessFixtureMatrixSchema},
		{Name: "schema-validation", Path: ".onlava/harness/schema-validation-latest.json", SchemaVersion: harnessSchemaValidationSchema},
		{Name: "agent-context", Path: ".onlava/harness/agent-context.json", SchemaVersion: harnessAgentContextSchema},
	}
}

func readLatestSelfHarness(root string) (harnessSelfResponse, bool, error) {
	path := filepath.Join(root, ".onlava", "harness", "self-latest.json")
	if _, err := os.Stat(path); err != nil {
