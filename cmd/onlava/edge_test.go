...
func TestCaddyEdgeConfigUsesStableAgentRouterContract(t *testing.T) {
	config := caddyEdgeConfig(caddyEdgeConfigOptions{
		...
		AdminSocket: "/tmp/onlava-caddy.sock",
		Token:       "secret-token",
	})
	for _, want := range []string{
		...
		"admin unix///tmp/onlava-caddy.sock",
		"strict_sni_host on",
		"https://:19443 {",
		...
		"header_up X-Onlava-Edge-Token secret-token",
	} {
...
func TestParseEdgeHelperLaunchStatusUsesTopLevelState(t *testing.T) {
	state, pid, err := parseEdgeHelperLaunchStatus(`system/dev.onlava.edge-helper = {
...
func TestEdgeAgentCommandMatchesSameSocketAndRouterOnly(t *testing.T) {
	...
	if edgeAgentCommandMatches(command, "/Users/petrbrazdil/.onlava/run/agent.sock", "127.0.0.1:9555") {
		t.Fatal("different router should not match")
	}
	if edgeAgentCommandMatches("/usr/bin/other --socket /Users/petrbrazdil/.onlava/run/agent.sock --router-listen 127.0.0.1:9440", "/Users/petrbrazdil/.onlava/run/agent.sock", "127.0.0.1:9440") {
		t.Fatal("non-onlava agent command should not match")
	}
}
...
func TestCaddyTrustConfigUsesAdminOnlyLocalCA(t *testing.T) {
	config := caddyTrustConfig("/tmp/onlava-trust.sock")
	for _, want := range []string{
		"local_certs",
		"admin unix///tmp/onlava-trust.sock",
	} {
		if !strings.Contains(config, want) {
...
func TestEdgeTrustUsesTemporaryCaddyAdmin(t *testing.T) {
	...
	t.Setenv("ONLAVA_FAKE_CADDY_MARKER", marker)
...
if [ "$cmd" = "run" ]; then
	echo run >> "$ONLAVA_FAKE_CADDY_MARKER"
...
if [ "$cmd" = "trust" ]; then
	echo trust >> "$ONLAVA_FAKE_CADDY_MARKER"
	exit 0
fi
exit 2
`
