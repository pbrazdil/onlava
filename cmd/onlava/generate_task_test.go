...
func TestSQLCGeneratorIgnoresSeedData(t *testing.T) {
	root := t.TempDir()
	writeSQLCFixture(t, root)
	writeTestAppFile(t, root, "auth/db/seed.sql", `insert into onlava_auth.users(id) values ('dev-user');
`)
...
`)
	...
	var out bytes.Buffer
	if err := runOnlavaInspect([]string{"generators", "--app-root", root, "--json"}, &out); err != nil {
		t.Fatalf("runOnlavaInspect(generators) returned error: %v", err)
	}
	var payload generatorGraphResponse
	if err := json.Unmarshal(out.Bytes(), &payload); err != nil {
		t.Fatalf("json.Unmarshal: %v\n%s", err, out.String())
...
func TestDBApplyRunsApplyWithoutSQLC(t *testing.T) {
  ...
  "generators": {
	...
	if payload.SchemaVersion != "onlava.db.apply.result.v1" || payload.Apply.Provider != "exec" || payload.Apply.Status != "applied" {
...
func TestDBSeedSafetyAllowsIdempotentInsertsAndUpserts(t *testing.T) {
	root := writeSeedCommandFixture(t)
	writeTestAppFile(t, root, "auth/db/seed.sql", `insert into onlava_auth.users(id) values ('dev-user');
insert into onlava_auth.users(id) values ('dev-user') on conflict (id) do update set id = excluded.id;
delete from onlava_auth.temp_users where id = 'dev-user';
`)
	store := newFakeSeedStore()
	restore := stubSeedStore(t, store)
	defer restore()
...
func TestDBSeedSafetyRejectsDestructiveStatements(t *testing.T) {
	tests := []struct {
		...
		{name: "drop", sql: "drop table onlava_auth.users;\n", message: "DROP"},
		{name: "truncate", sql: "truncate table onlava_auth.users;\n", message: "TRUNCATE"},
		{name: "delete without where", sql: "delete from onlava_auth.users;\n", message: "broad DELETE"},
		{name: "delete where true", sql: "delete from onlava_auth.users where true;\n", message: "broad DELETE"},
		{name: "delete one equals one", sql: "delete from onlava_auth.users where 1 = 1;\n", message: "broad DELETE"},
	}
...
func TestDBSeedSafetyIgnoresCommentsAndStrings(t *testing.T) {
	root := writeSeedCommandFixture(t)
	writeTestAppFile(t, root, "auth/db/seed.sql", `-- drop table onlava_auth.users;
/* truncate table onlava_auth.users; */
insert into onlava_auth.audit(message) values ('delete from onlava_auth.users;');
insert into onlava_auth.audit(message) values ($$drop table onlava_auth.users;$$);
`)
	store := newFakeSeedStore()
	restore := stubSeedStore(t, store)
	defer restore()
...
func writeSeedCommandFixture(t *testing.T) string {
	t.Helper()
	root := t.TempDir()
	writeTestAppFile(t, root, ".onlava.json", `{"name":"seedapp"}`)
