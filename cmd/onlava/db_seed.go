...
func buildDBSeedResultWithEnv(ctx context.Context, appRoot string, cfg appcfg.Config, opts dbSeedOptions, baseEnv []string, useManaged bool) (dbSeedResult, error) {
	result := dbSeedResult{
		SchemaVersion: "onlava.db.seed.result.v1",
...
func renderDBSeedText(stdout io.Writer, result dbSeedResult) {
	if len(result.Seeds) == 0 {
		fmt.Fprintln(stdout, "onlava: no seed files discovered")
		return
	}
	...
	fmt.Fprintf(stdout, "onlava: database seed complete; planned=%d applied=%d skipped=%d changed=%d failed=%d\n",
...
create schema if not exists onlava_internal;
create table if not exists onlava_internal.seed_runs (
...
func (s *pgxDatabaseSeedStore) LookupSeed(ctx context.Context, appID, path string) (string, bool, error) {
	var hash string
	err := s.conn.QueryRow(ctx, `select sha256 from onlava_internal.seed_runs where app_id = $1 and path = $2`, appID, path).Scan(&hash)
...
func (s *pgxDatabaseSeedStore) ApplySeed(ctx context.Context, appID, path, hash, sql string) error {
	...
	if _, err := tx.Exec(ctx, `insert into onlava_internal.seed_runs (app_id, path, sha256) values ($1, $2, $3)`, appID, path, hash); err != nil {
		return err
	}
