...
func TestOpenSubstrateLogWritersWritesSeparateFiles(t *testing.T) {
	...
	if !strings.HasSuffix(logs.stdoutPath, filepath.Join(".onlava", "substrates", "victoria", "logs", "victoria.traces.stdout.log")) {
...
func TestSubstrateExitRecordCapturesExitCodeAndLogPaths(t *testing.T) {
	if os.Getenv("ONLAVA_SUBSTRATE_EXIT_HELPER") == "1" {
		os.Exit(7)
	}
	cmd := exec.Command(os.Args[0], "-test.run=TestSubstrateExitRecordCapturesExitCodeAndLogPaths")
	cmd.Env = append(os.Environ(), "ONLAVA_SUBSTRATE_EXIT_HELPER=1")
