...
const harnessSelfSummarySchema = "onlava.harness.self.summary.v1"
...
func summarizeDrift(report *harnessDriftReport) *harnessSelfDriftSummary {
	...
	return &harnessSelfDriftSummary{EnvVarCount: len(report.Env.Variables), Diagnostics: len(report.Diagnostics), CLICommandCount: len(report.CLI.Commands), EmbedCount: len(report.Embeds.Embeds), Artifact: ".onlava/harness/drift-latest.json"}
}
...
func summarizeTestTiming(report *harnessTestTimingReport) *harnessSelfTestTimingSummary {
	...
	return &harnessSelfTestTimingSummary{
		...
		Artifact:        ".onlava/harness/test-timing-latest.json",
	}
}

func summarizeKnowledge(resp harnessSelfResponse) harnessSelfKnowledgeSummary {
	out := harnessSelfKnowledgeSummary{EntrypointCount: len(resp.Knowledge.Entrypoints), SchemaCount: len(resp.Knowledge.Schemas), Drilldown: "onlava inspect docs --json"}
...
func summarizeArchitecture(steps []harnessStep, changedPaths map[string]bool, debtWarnings, changedWarnings int) harnessSelfArchitectureSummary {
	out := harnessSelfArchitectureSummary{DebtWarningCount: debtWarnings, ChangedAreaWarningCount: changedWarnings, Artifact: ".onlava/harness/self-latest.json"}
	for _, step := range steps {
...
func summarizeSchemaValidation(report *harnessSchemaValidationReport) *harnessSelfSchemaValidationSummary {
	...
	out := &harnessSelfSchemaValidationSummary{Artifact: ".onlava/harness/schema-validation-latest.json"}
...
func summarizeFixtureMatrix(report *harnessFixtureMatrixReport) *harnessSelfFixtureMatrixSummary {
	...
	out := &harnessSelfFixtureMatrixSummary{Artifact: ".onlava/harness/fixture-matrix-latest.json"}
...
func artifactForStepName(name string) string {
	...
	case "contract drift checks":
		return ".onlava/harness/drift-latest.json"
	case "fixture matrix":
	...
	case "toolchain preflight":
		return ".onlava/harness/toolchain-latest.json"
	default:
		return ".onlava/harness/self-latest.json"
	}
}
...
func drilldownForStepName(name string) string {
	switch name {
	case "go tests":
		return "onlava inspect harness timing --top 10 --json"
	case "contract drift checks":
		return "onlava inspect harness artifact drift --json"
	default:
		return "onlava inspect harness diagnostics --severity warning --json"
	}
}
...
func normalizeLikelyPath(path string) string {
	...
	path = filepath.ToSlash(path)
	if idx := strings.Index(path, "/.onlava/"); idx >= 0 {
		return ".onlava/" + strings.TrimPrefix(path[idx+len("/.onlava/"):], "/")
	}
