...
The best first production-ready Pulse should be:
...
Everything else should either be explicitly beta or moved behind pulse dev:
...
Right now pulse run creates a dev supervisor, starts the dashboard, starts DB Studio, and starts the local proxy path:

* cmd/pulse/watch.go:36-79 calls newDevSupervisor, supervisor.Start, and then rebuilds/restarts the app.
* cmd/pulse/dev_supervisor.go:193-210 starts dashboard, DB Studio, and local HTTPS proxy.
* internal/codegen/generator.go:185-193 generates app mains that import _ "pulse.dev/runtimeapp".
...
You can still keep pulse run --dashboard, but the default should be headless and predictable.
...
* cmd/pulse/dashboard.go:27 imports pulse.dev/ui.
* cmd/pulse/dashboard.go:250-257 calls fs.Sub(uidist.Dist, "dist").

That means a normal Go build of the CLI should fail unless the UI has already been built into ui/dist. Either commit the built dashboard assets, generate them as part of release packaging, or move the UI embed behind a build step/tag. Do not ship until go install ./cmd/pulse works from a clean checkout.
...
go install ./cmd/pulse from clean checkout
pulse check --json on fixtures
pulse run --json on fixtures
pulse build on fixtures
...
* /__pulse/config at runtime/server.go:67-79
* /__pulse/pubsub/clear at runtime/server.go:81-103
...
* internal/localproxy/proxy.go:63-65 defaults PULSE_LOCAL_PROXY to enabled.
...
* cmd/pulse/dev_supervisor.go:778-805 starts the local proxy unless PULSE_LOCAL_PROXY=0.
...
* Those wrappers call pulseruntime.CallEndpoint, so a normal-looking Go call can behave like a Pulse runtime call.
...
// Pulse RPC semantics use an explicit generated client.
...
* cmd/pulse/dev_supervisor.go:505-529 also loads .env for child process env.
* internal/dbstudio/dbstudio.go has its own .env parser.
* cmd/pulse/dev_supervisor.go validates .env.local, but the main runtime loaders do not appear to load .env.local.
...
include:
  ...
  generated Pulse files
exclude:
  .env
  .env.*
  .git
  .pulse runtime state unless explicitly needed
  node_modules
...
Then either fully honor encoding/json behavior or clearly document Pulse’s custom response-shaping semantics.
...
cmd/pulse/watch.go:299-311 watches only:
...
The actual CLI in cmd/pulse/main.go:34-52 includes:
...
Directives:
  ...
  //pulse:service
  //pulse:authhandler
  //pulse:middleware only if middleware ordering/semantics are fully tested
Runtime:
  typed endpoints
  raw endpoints
...
CLI:
  ...
  pulse build
  pulse check --json
  pulse inspect app|routes|services|build|paths --json
  pulse logs --jsonl
  pulse test
  pulse gen client --lang typescript
Generated artifacts:
  .pulse/gen/app.json
...
Build/release:
  clean checkout can run go install ./cmd/pulse
  no missing ui/dist embed failure
  no .DS_Store / __MACOSX in release archive
  version command exists: pulse version --json
