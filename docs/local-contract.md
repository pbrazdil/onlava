...
- Session owner checks treat `owner_pid` as the effective owner. `owner.pid` is the fingerprint for that same PID, not an independent owner field. If the stored owner fingerprint object points at a different stale PID, Onlava refreshes it on the next registration and must not delete or prune the session while the effective `owner_pid` is still live. Dev supervisors unregister sessions with an owner-conditional delete; if an older owner exits after ownership moved, the delete is ignored and the newer session record remains registered.
- `onlava status --json` treats a `starting` or `running` session with a missing or mismatched effective owner as stale instead of reporting it as healthy.
...
- Managed Electric processes are session-owned children. They receive Onlava app-root/session identity in their environment and are recorded in the agent session process map so a later owner can clean stale Electric processes for the same app-root/session without touching other sessions. Before starting Electric, onlava checks live process command lines for the exact `ELECTRIC_REPLICATION_STREAM_ID=<session-stream-id>` stream. It terminates Onlava-owned same app-root/session Electric processes and fails fast with PID/command diagnostics for any remaining process using that stream. Before starting Electric against managed Postgres, onlava also checks for advisory-lock or replication-slot backends for the exact `electric_slot_<session-stream-id>` slot, terminates stale matching Postgres backends, and reports the remaining backend PID/query details if cleanup cannot clear the slot.
