...
- `proxy.frontends` is a map keyed by frontend name. Each frontend requires `host`; `root` defaults to `apps/<name>`; `upstream` is optional but ignored by agent dev unless that frontend also sets `allow_shared_upstream: true`. With an active agent, `onlava dev` prefers to start supported Vite/Astro frontends on hidden loopback ports, inject routed API/Electric URLs into their process environment, register those hidden ports as session backends, and expose `https://<frontend>.<session>.onlava.localhost:<agent-router-port>/` by default. Managed frontend session routes serve the frontend shell for HTML SPA deep links, while `/__onlava/*`, `/api/*`, `/sync/*`, and concrete asset paths are not history-fallback routes. `ONLAVA_FRONTEND_<NAME>_ADDR` still overrides onlava-owned frontend startup for manual debugging.
...
- `onlava status --json` treats a `starting` or `running` session with a missing or dead effective owner as `stale`, and a live but fingerprint-mismatched owner, dead app PID, or duplicate same app-root/session owner process as `degraded`. Status JSON includes `status_reason` when onlava rewrites the session status.
