...
scenery needs a small, boring local script runner for domain-owned operational scripts such as data backfills, reconciliations, one-off inspections, and fixture loaders. The runner should let contributors invoke a script by app-local namespace:

```sh
scenery run billing:reconcile --dry-run --limit 100
```

The important constraint is isolation from normal app parsing and builds. Go scripts placed under an app root can otherwise be discovered by `packages.Load("./...")`, and multiple `package main` files in one directory can break type checking before scenery ever decides to ignore them. This feature must therefore define script conventions that are safe before package loading happens.

The final behavior should support simple single-file scripts and robust directory-per-script layouts for both Go and TypeScript, without coupling script discovery to scenery service discovery.
...
- [x] 2026-06-01: Added `scenery run list`, `scenery run inspect`, and `scenery run <domain>:<script>`.
- [x] 2026-06-01: Moved headless API execution to `scenery serve`, leaving `scenery run` dedicated to scripts.
...
- 2026-06-01: Adding `.scenery.json` to `testdata/apps/script-fixture` made the self-harness fixture matrix treat it as a normal app. The fixture now includes a minimal public API package so script-runner coverage and normal app fixture checks both pass.
- 2026-06-01: `scenery harness self --json --write` now passes the script fixture matrix, but the overall harness remains red on the existing full-suite timing budget: this run reported 10.700s against the 7.000s target and points to `docs/plans/0050-test-suite-speed-hardening.md`.

## Decision Log

- Decision: Keep all local operational script commands under `scenery run`.
  Rationale: `scenery serve` now owns headless API execution, so `run` can be the ergonomic operational-script verb and discovery namespace without keeping a second `script` command.
...
Implemented a narrow script runner in `cmd/scenery` with filesystem-first discovery, strict `<domain>:<script>` target parsing, ambiguity errors, Go and TypeScript layout support, and process execution from the app root. The command surface is `scenery run list|inspect` plus `scenery run <domain>:<script>` for executing a script target.
...
- Focused fixture scenarios passed:
  - `scenery run list --app-root testdata/apps/script-fixture --json`
  - `scenery run inspect billing:reconcile --app-root testdata/apps/script-fixture --json`
  - `scenery run --app-root testdata/apps/script-fixture billing:reconcile --dry-run`
- `scenery harness self --json --write` wrote `.scenery/harness/self-latest.json`; all feature-relevant checks and the fixture matrix passed, while the overall harness remained red on the pre-existing full-suite timing budget.
...
- `scenery serve` is the headless app runner; `scenery run` executes operational scripts.
- `internal/app/root.go` discovers the app root by walking to `.scenery.json`.
...
The domain is a top-level path segment, not a Scenery service name.
...
```sh
scenery run list [--app-root <path>] [--json]
scenery run inspect <domain>:<script> [--app-root <path>] [--lang go|typescript] [--json]
```

Add sugar:

```sh
scenery run [--app-root <path>] [--env <name>] [--lang go|typescript] <domain>:<script> [script args...]
```

Acceptance:

- `scenery run billing:reconcile --dry-run` executes the `billing:reconcile` script.
...
- Runtime selection follows scenery's TypeScript worker convention: prefer Bun, otherwise use Node with `tsx`.
...
Start with a small internal script model in `cmd/scenery` or an internal package if reuse becomes useful. Keep the first implementation local to the CLI unless another package needs the resolver.

Implement target parsing and filesystem discovery before adding execution. Once the resolver is covered by tests, add `scenery run list`, `scenery run inspect`, and `scenery run <domain>:<script>` execution.
...
4. Add `scenery run list` and `scenery run inspect`.
5. Add Go execution with build-tag validation for `*.script.go`.
6. Add TypeScript execution with Bun/Node runtime selection.
7. Add top-level `scenery run <domain>:<script>` and argument-splitting tests.
...
- `run` only executes user-authored script code and should not mutate scenery state by itself.
- If a script process exits non-zero, scenery returns that failure without cleaning or modifying source files.
- Re-running a script command should not create persistent scenery artifacts except normal build/cache artifacts created by Go, Bun, Node, or existing scenery helpers.
...
```text
docs/plans/0058-scenery-script-runner.md
docs/plans/active.md
cmd/scenery/main.go
cmd/scenery/serve.go
cmd/scenery/script*.go
cmd/scenery/script*_test.go
...
- Coupling script names to scenery service discovery.
...
- Reuse existing scenery helpers for app-root discovery and TypeScript runtime selection where practical.
