...
scenery now has the right UI architecture direction: an `@scenery` shadcn registry, a guarded shadcn wrapper, scenery-owned primitives and layouts, a UI agent contract, and self-harness static checks. Before building larger UI surfaces such as a dashboard data explorer, harden the guardrails so they are reliable enforcement rather than mostly guidance.

Do not undo the `@scenery` registry model. Do not build new product UI in this plan. The goal is to close the small but important bypasses in the wrapper, registry item validation, import scanning, and className drift detection.
...
wrapper:
  ...
  serves only local scenery registry items
...
After this plan, `0013 Dashboard Data Explorer` can safely use `DataExplorerLayout` as the proving ground for the scenery UI composition model.
...
- [x] (2026-05-09 12:33Z) Made `ui/scripts/scenery-shadcn.mjs` fail hard when port `127.0.0.1:4873` is occupied.
- [x] (2026-05-09 12:33Z) Made the wrapper reject every unknown flag; only `--dry-run`, `--overwrite`, and `-o` are accepted.
- [x] (2026-05-09 12:33Z) Pinned the shadcn CLI version through `SCENERY_SHADCN_VERSION ?? "shadcn@4.7.0"`.
...
- [x] (2026-05-09 12:36Z) Added `PageToolbar` and the `@scenery/page-toolbar` registry item.
...
- Official shadcn docs say target aliases in `files[].target` were added in `shadcn@4.7.0`, including alias targets such as `@ui/ai/prompt-input.tsx`. This makes a pinned CLI version important because scenery registry items already rely on `target` alias behavior.
- Official shadcn registry item docs allow `registryDependencies` to be names, namespaces, or URLs, and allow `files[].target` to write to root-relative paths such as `~/.env`. scenery must explicitly forbid those capabilities in the approved registry.
- Current `ui/scripts/scenery-shadcn.mjs` treats `EADDRINUSE` as success by returning a dummy `close` function. That can make the wrapper proceed against an unknown process already listening on `127.0.0.1:4873`.
...
- Decision: Keep `@scenery/*` and the shadcn wrapper, but make wrapper behavior stricter before building more UI.
...
- Decision: Pin `shadcn@4.7.0` or newer in the wrapper.
  Rationale: scenery's registry model depends on CLI semantics, especially target aliases. `latest` is convenient but undermines deterministic guardrails.
  Date/Author: 2026-05-09 / Codex
...
- The shadcn wrapper rejects occupied registry port, unknown flags, unsafe overwrite, non-`@scenery` items, URLs, and local path-style items before invoking shadcn.
- The wrapper defaults to pinned `shadcn@4.7.0` while preserving `SCENERY_SHADCN_VERSION` as an explicit escape hatch.
...
- `PageToolbar` is implemented and exported, with an `@scenery/page-toolbar` registry item.
...
cd ui && bun run shadcn:add @scenery/button --foo       # expected failure
cd ui && bun run shadcn:add @scenery/button --cwd .     # expected failure
...
This plan is for the `scenery.sh` repository.

Read these files first:

- `docs/plans/0011-scenery-ui-registry-and-agent-guardrails.md`
...
```text
file: ui/scripts/scenery-shadcn.mjs
...
```text
file: cmd/scenery/harness_ui.go
...
- Registry item `files[].target` may also point to root-relative locations such as `~/.env`; scenery should reject those for the approved registry.
- `registryDependencies` can include un-namespaced names, namespaced items, or URLs; scenery should allow only `@scenery/*`.
...
Change `ui/scripts/scenery-shadcn.mjs` so it is deterministic and conservative:
...
- allow --dry-run
- allow --overwrite/-o only when SCENERY_SHADCN_OVERWRITE=1
- pin CLI to SCENERY_SHADCN_VERSION or shadcn@4.7.0
```
...
Extend `checkUIRegistryItems` in `cmd/scenery/harness_ui.go` so registry files are treated as write-capability declarations.

Validate every `files[]` entry:

```text
source:
  required when scenery uses local source expansion
  must exist
...
target:
  required for scenery registry items
...
Add focused tests for `cmd/scenery` UI static check behavior. Prefer Go tests in `cmd/scenery` using temporary fixture trees. Tests should cover:

```text
- raw shadcn script rejected
- non-@scenery registry rejected
...
For source validation, scenery's current registry item JSON uses `source` as a Scenery wrapper extension. The source path is resolved relative to `ui/registry/scenery` by the wrapper. The harness should validate both the source and the target so local registry metadata cannot write surprising files.
...
1. Edit `ui/scripts/scenery-shadcn.mjs`:
   - replace `shadcn@latest` with `process.env.SCENERY_SHADCN_VERSION ?? "shadcn@4.7.0"`
   - reject `EADDRINUSE`
   - remove `--cwd` support
...
3. Extend `cmd/scenery/harness_ui.go` registry item structs and validation.
4. Add tests in `cmd/scenery` for registry validation failures.
...
10. Add `ui/src/components/layouts/PageToolbar.tsx`, export it from `ui/src/components/layouts/index.ts`, and add an `@scenery/page-toolbar` registry item if useful.
...
Run from the scenery repo root unless specified:
...
cd ui && bun run shadcn:add @scenery/button --foo
cd ui && bun run shadcn:add @scenery/button --cwd .
```
...
cmd/scenery/harness_ui.go
cmd/scenery/harness_ui_test.go
docs/ui-agent-contract.md
docs/local-contract.md
docs/harness-engineering.md
...
```json
{
  ...
  "severity": "error",
  "file": "/repo/ui/registry/scenery/bad.json",
...
SCENERY_SHADCN_OVERWRITE=1
SCENERY_SHADCN_VERSION
SCENERY_SHADCN_REGISTRY_ROOT
scenery harness self --json --write
