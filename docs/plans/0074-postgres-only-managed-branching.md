...
-> DB setup/apply/seed
-> Electric, auth, and dev harnesses work
...
- [x] Preserve app-facing `DatabaseURL` injection, DB lifecycle, Electric, auth, and running ONLV behavior for the Postgres branch path.
...
- 2026-06-10: ONLV's `scenery up --json --detach` reached `running` with API, Electric, frontends, and TypeScript worker registered. Logs showed `database branch lease ready` from provider `postgres`, and `curl -k https://api.main-dbe32e.onlv.dev/healthy` returned HTTP 200.
...
branch template, and a sibling Electric service. The app-facing contract is the
...
database, drops owned objects and the role, cleans up Electric publications or
...
database setup/apply/seed, auth bootstrap, Electric startup, workers, and
harnesses must keep consuming the same app-facing managed database URL contract.
Electric must receive a deterministic session- or branch-scoped replication
...
and Electric-friendly logical replication settings. Add only the branch-specific
...
logical replication behavior needed by Electric.
...
runs, Electric starts against the branch database, and `uuidv7()` is available
on PostgreSQL 18.
...
8. Update Electric/logical-replication ownership. Ensure local Postgres starts
...
logical-replication-smoke-for-electric
...
- Postgres logical replication supports Electric-like publish/subscribe flows,
...
- `scenery db branch status|list|checkout|reset|delete|restore|diff|expire|prune`.
- `scenery db psql`.
- `scenery up`, DB setup/apply/seed, auth bootstrap, Electric startup, workers,
...
- Electric process/container startup and replication stream naming.
