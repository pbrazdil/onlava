...
scenery currently uses embedded Caddy modules in `internal/localproxy` to serve local HTTPS reverse-proxy routes for `scenery dev` and standalone `scenery run`. The goal is to replace that embedded Caddy dependency with a small scenery-native Go implementation that preserves the current public `localproxy` API and user-visible behavior.

When this plan is complete, `scenery dev` and standalone runtime development mode still expose HTTPS URLs such as `https://api.<workspace>.localhost`, `https://console.<workspace>.localhost`, `https://removed-agent-transport.<workspace>.localhost`, and `https://scenery.<workspace>.localhost`, but the implementation uses only the Go standard library for routing, TLS certificates, trust installation, reverse proxying, and lifecycle management. Caddy imports and Caddy-only dependencies are removed from the repository.

## Progress

- [x] (2026-04-27 17:06Z) Created this ExecPlan and assigned historical ID 0004.
- [x] (2026-04-27 17:19Z) Replaced Caddy-backed route/config generation with a Scenery-native route table.
- [x] (2026-04-27 17:19Z) Implemented scenery local CA, leaf certificate generation, and SAN validation.
...
- [x] (2026-04-27 17:22Z) Ran full validation: `go test ./...`, `go install ./cmd/scenery`, Caddy dependency checks, and `scenery harness self --json --write`.
- [x] (2026-04-27 18:11Z) Corrected `scenery dev` startup so local HTTPS domains are enabled by default, with `SCENERY_LOCAL_PROXY=0|false|no|off` as the opt-out.
...
- [x] (2026-04-27 18:27Z) Corrected `scenery dev` trust setup so the generated local CA is installed by default, with `SCENERY_LOCAL_PROXY_SKIP_TRUST_INSTALL=1` as the opt-out.
...
- Current `internal/localproxy/proxy_test.go` names local proxy and trust defaults as opt-in: `Enabled()` currently defaults to `false`, and `SkipInstallTrust()` currently defaults to `true`. The requested target behavior says `SCENERY_LOCAL_PROXY` should default to enabled and `SCENERY_LOCAL_PROXY_SKIP_TRUST_INSTALL` should default to false. Implementation must decide whether to update behavior to the requested target and adjust tests, or preserve actual current behavior if callers rely on it. Record the final decision in the Decision Log.
- Current `runtimeapp/app.go` only disables standalone proxy startup when `SCENERY_LOCAL_PROXY == "0"`. The requested behavior allows replacing this with `localproxy.Enabled()` only if tests cover `"0"`, `"false"`, `"no"`, and `"off"`.
- Current Caddy setup passes both `HTTPPort` and `HTTPSPort` into the Caddy HTTP app but configures only the `scenery` server with `Listen: :<HTTPSPort>`. The replacement should implement an HTTP redirect listener unless implementation investigation proves Caddy did not bind HTTP in this configuration.
...
- After the first native-proxy pass, `scenery dev` without `--proxy` still printed loopback URLs because the CLI only enabled `SCENERY_LOCAL_PROXY` from the explicit flag. `scenery dev` now enables the proxy by default while preserving an environment opt-out.
...
- After domains appeared in the banner, Chromium still showed `ERR_CERT_AUTHORITY_INVALID` because `scenery dev` was setting `SCENERY_LOCAL_PROXY_SKIP_TRUST_INSTALL=1` by default. The CLI now defaults that env var to `0` for `scenery dev`.
- The Darwin `security add-trusted-cert` invocation imported the CA into the login keychain, but `security dump-trust-settings` showed no scenery CA user trust settings. Removing `-d` and adding explicit `-p ssl -p basic` records user trust and makes system certificate verification accept the local domains.

## Decision Log

- Decision: Keep the replacement inside `internal/localproxy` and preserve all exported names from that package.
  Rationale: `cmd/scenery/dev_supervisor.go` and `runtimeapp/app.go` already depend on `localproxy.Config`, `Start`, `Proxy.Routes`, and URL helper functions. Preserving the API keeps the migration focused on implementation parity.
...
- Decision: Keep standalone runtime proxy startup enabled by default inside `SCENERY_STANDALONE_DEV`, but treat `SCENERY_LOCAL_PROXY=0`, `false`, `no`, and `off` as disabled.
...
- Decision: Enable the local HTTPS proxy by default for `scenery dev`, while keeping `internal/localproxy.Enabled()` opt-in for library callers and respecting `SCENERY_LOCAL_PROXY=0`, `false`, `no`, or `off` as a dev opt-out.
  Rationale: `scenery dev` is the full local development platform and should surface the scenery local domains without requiring `--proxy`; the package-level default remains conservative for non-CLI callers.
  Date/Author: 2026-04-27 / Codex

- Decision: Install local CA trust by default for `scenery dev`, while respecting `SCENERY_LOCAL_PROXY_SKIP_TRUST_INSTALL=1` as an opt-out and keeping `--trust` as an explicit force-on flag.
  Rationale: Local HTTPS domains are only useful in the browser when the generated scenery CA is trusted. The package-level default remains conservative, but `scenery dev` should provide the complete local development platform.
  Date/Author: 2026-04-27 / Codex

- Decision: On Darwin, install scenery's local CA as a user trust root with explicit SSL and basic X.509 policies.
...
Completed on 2026-04-27. scenery local HTTPS proxying now uses a small standard-library implementation for route matching, TLS certificate generation, trust installation hooks, reverse proxying, redirects, and lifecycle cleanup. Caddy, CertMagic, and ZeroSSL are no longer present in `go.mod`, `go.sum`, `go list -m all`, or `internal/localproxy` imports.

Validation passed with `go test ./internal/localproxy`, `go test ./runtimeapp ./cmd/scenery`, `go test ./...`, `go install ./cmd/scenery`, and `scenery harness self --json --write`. The self harness still reports the pre-existing `.DS_Store` warning, but no errors.
...
- `cmd/scenery/dev_supervisor.go`: starts the proxy before launching the child app, sets `SCENERY_LOCAL_PROXY=0` in the child, and sets `SCENERY_PUBLIC_BASE_URL` when a proxy exists.
...
Milestone 1 introduces a Caddy-independent route table. This is complete when tests can inspect resolved API, console, removed agent transport, frontend, and `/__scenery/config` routes without building Caddy JSON.

Milestone 2 implements local certificate storage. This is complete when the package can generate or reuse a Scenery development CA, generate a leaf certificate with the expected SAN DNS names, store private keys with `0600`, store directories with `0700`, and regenerate leaf certificates when missing, expired, near expiry, not covered by SANs, or signed by a changed CA.
...
Start by carving the route model out of the Caddy JSON model. Replace `configJSON`, `proxyRoutes`, and `proxyRoute` with small route structs such as `proxyRoute{host, path, upstream string, rewriteHost bool}` and a route table that matches host case-insensitively after stripping any port from `req.Host`. The frontend host must register the exact `/__scenery/config` route before the catch-all frontend route so API config routing wins.

Next add certificate code under `internal/localproxy/cert.go`. Use `SCENERY_DEV_CACHE_DIR` when set, otherwise `os.UserCacheDir()`, and store files under a Scenery-specific directory like `<cache-root>/scenery/localproxy/`. Generate an ECDSA P-256 or RSA 2048 CA certificate, a matching CA key, a leaf certificate, and a leaf key. Keep the CA long-lived, keep the leaf shorter-lived, and expose small internal helpers so tests can load the generated CA into an `http.Client` root pool without touching the system trust store.
...
Then implement serving with `net/http` and `net/http/httputil.ReverseProxy`. Build upstream targets as `http://<normalized-upstream>`, preserve path and query, return `502` on upstream errors, return `404` for unknown host/path combinations, and keep default reverse proxy streaming behavior for SSE and WebSocket traffic. For API, console, removed agent transport, and frontend `/__scenery/config`, preserve the incoming `Host` header. For the frontend catch-all route, set `req.Host` to the upstream host:port to match the old Caddy `Host: {http.reverse_proxy.upstream.hostport}` behavior. Set useful `X-Forwarded-For`, `X-Forwarded-Host`, and `X-Forwarded-Proto`; HTTPS requests should report `X-Forwarded-Proto: https`.

Finally update the call sites only as needed. `cmd/scenery/dev_supervisor.go` should continue to start the proxy before the child app and pass `SCENERY_LOCAL_PROXY=0` plus `SCENERY_PUBLIC_BASE_URL=<routes.APIURL>`. `runtimeapp/app.go` should continue to avoid a second proxy when launched by the supervisor or when local proxy is disabled. The runtime banner should still use routes for API, dashboard, removed agent transport SSE, frontend, URLs.

## Concrete Steps

1. Read current `internal/localproxy/proxy.go`, `internal/localproxy/proxy_test.go`, `cmd/scenery/dev_supervisor.go`, `runtimeapp/app.go`, and `go.mod`.
...
10. Update environment tests for `SCENERY_LOCAL_PROXY`, `SCENERY_LOCAL_PROXY_HTTP_PORT`, `SCENERY_LOCAL_PROXY_HTTPS_PORT`, `SCENERY_LOCAL_PROXY_SKIP_TRUST_INSTALL`, named `SCENERY_FRONTEND_<NAME>_ADDR`, and `SCENERY_DISABLE_FRONTEND_PROXY`.
...
13. Update `docs/plans/0004-scenery-native-localproxy.md` progress and discoveries as each milestone lands.
...
Focused validation during implementation:
    ...
    go test ./cmd/scenery ./runtimeapp
...
Full repository validation before finishing:
    ...
    gofmt -w internal/localproxy/*.go cmd/scenery/dev_supervisor.go runtimeapp/app.go
    go test ./...
...
Manual smoke validation when practical:
    ...
    SCENERY_LOCAL_PROXY=1 SCENERY_LOCAL_PROXY_SKIP_TRUST_INSTALL=1 SCENERY_LOCAL_PROXY_HTTPS_PORT=9443 scenery run

Run this from a fixture app or a read-only app root. Confirm that the banner still prints HTTPS API, dashboard, removed agent transport SSE, frontend, URLs as applicable. Also verify that `SCENERY_LOCAL_PROXY=0 scenery run` avoids starting the proxy, that custom `SCENERY_LOCAL_PROXY_HTTPS_PORT` appears in route URLs, and that explicit proxy hosts from `.scenery.json` override workspace-derived hosts.
...
- New integration tests prove HTTPS reverse proxy parity for API, console, removed agent transport, frontend, `/__scenery/config`, frontend `Host` rewrite, unknown host `404`, HTTP redirect, and `Close` lifecycle.
- TLS certificates contain the expected SAN DNS names and work with clients that trust the generated scenery local CA.
...
- Each configured frontend host routes exact path `/__scenery/config` to `APIUpstream` and preserves incoming `Host`.
...
Environment variables to preserve:
    ...
    SCENERY_LOCAL_PROXY
    SCENERY_LOCAL_PROXY_HTTP_PORT
    SCENERY_LOCAL_PROXY_HTTPS_PORT
    SCENERY_LOCAL_PROXY_SKIP_TRUST_INSTALL
    SCENERY_FRONTEND_<NAME>_ADDR
    SCENERY_DISABLE_FRONTEND_PROXY
    SCENERY_DEV_CACHE_DIR
