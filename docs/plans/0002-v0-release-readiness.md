...
release-hardening context. The current command contract uses `scenery up` for the
local app session and `scenery serve` for headless API execution; use
`docs/local-contract.md` as the source of truth.

## Purpose / Big Picture

scenery is close to being a useful local-first runtime, but the current repository mixes stable app runtime behavior with development-platform behavior. The first production-ready release should be intentionally smaller, more boring, and easier to validate.
...
- [x] (2026-04-27 17:36Z) Confirmed the `scenery dev` / headless `scenery run` split is implemented and kept [0001-devrun-command-split.md](0001-devrun-command-split.md) as the detailed dependency.
- [x] (2026-04-27 17:36Z) Confirmed current checkout has `ui/dist`, added `scenery version --json`, and added `scenery.version.v1` schema.
...
- [x] (2026-04-27 17:36Z) Made local HTTPS proxy and trust-store installation opt-in through `scenery dev --proxy` and `scenery dev --proxy --trust`.
- [x] (2026-04-27 17:36Z) Documented scenery-native behavior as stable.
...
- [x] (2026-04-27 17:36Z) Restricted build workspace copying so `.env`, `.env.*`, `.git`, `.scenery`, `node_modules`, `.DS_Store`, `__MACOSX`, and `coverage` are not persisted in build caches.
- [x] (2026-04-27 17:36Z) Added response JSON semantics tests for `json:"-"`, `omitempty`, embedded structs, headers, `scenery:"httpstatus"`, and custom marshalers.
- [x] (2026-04-27 17:36Z) Aligned CLI usage, docs, schemas, and implementation for the release-hardening slice.
- [x] (2026-04-27 18:38Z) Made missing declared secrets warn in local development but fail before serving under `scenery run --env production`.
- [x] (2026-04-27 18:48Z) Reclassified reachable-but-not-frozen surfaces so `scenery psql`, Victoria sidecars/downloads, Pub/Sub/Cron admin affordances, and trace/metric inspect semantics do not accidentally become stable v0 API.
...
- `scenery run` previously started development supervisor behavior, including dashboard, local HTTPS proxy, removed agent transport, and file watching.
- Generated app binaries could carry dev-platform behavior through `scenery.sh/runtimeapp`.
...
- The repo had conflicting guidance about strict scenery-only behavior versus migration compatibility support.
...
- `scenery dev` and headless `scenery run` were already split before this plan execution started, including tests that reject dev flags on `scenery run`.
- Runtime dev endpoints could be preserved for `scenery dev` by injecting `SCENERY_DEV_ENDPOINTS=1` into the development child process.
- Local proxy startup could be made opt-in without changing Caddy configuration internals by changing defaults and adding `scenery dev --proxy` / `--trust` environment wiring.
- Full integration tests exposed older assumptions that `scenery run` reflected arbitrary CORS origins by default and that `scenery dev` always started the local HTTPS proxy. The tests were updated to assert the new release contract: explicit CORS allowlist for headless run and `scenery dev --proxy` for hostname routing.
...
- The implemented CLI surface is broader than the stable support surface. `scenery psql`, Victoria sidecars/downloads, local admin clear commands, and trace/metric inspect subjects are useful, but their semantics are still too implementation-shaped to freeze as stable v0.
...
- Decision: Treat the command split in [0001-devrun-command-split.md](0001-devrun-command-split.md) as a release-readiness dependency, not a duplicate workstream.
  Rationale: `scenery dev` versus headless `scenery run` is the highest-leverage boundary and already has its own detailed ExecPlan.
  Date/Author: 2026-04-27 / Codex

- Decision: Stable v0 should prefer scenery-native behavior, with any migration tooling kept explicit and separate.
  Rationale: Hidden compatibility makes APIs harder to freeze and contradicts the repository’s strict scenery naming goal.
  Date/Author: 2026-04-27 / Codex
...
- Decision: Keep dev endpoints available under explicit dev mode instead of deleting them.
  Rationale: The dashboard and frontend proxy still need `/__scenery/config`, `/__scenery/pubsub/clear`, platform stats, and pprof during local development, but production-like `scenery run` and generated binaries should not expose them by default.
  Date/Author: 2026-04-27 / Codex

- Decision: Make local proxy trust installation a second opt-in after proxy startup.
  Rationale: Starting a local reverse proxy is less invasive than mutating system trust stores. `scenery dev --proxy` should not imply trust-store mutation.
  Date/Author: 2026-04-27 / Codex
...
- Decision: Missing declared secrets are warnings outside production and startup errors in production.
  Rationale: Local development should stay forgiving, but `scenery run --env production` must fail before serving if an app declares secrets that are not present in process env or `.env`.
  Date/Author: 2026-04-27 / Codex

- Decision: Classify implemented local helpers separately from stable v0 API.
  Rationale: Being reachable from the CLI should not imply a long-term support promise. `scenery psql`, Victoria supervision/download behavior, Pub/Sub/Cron admin affordances, and trace/metric inspection semantics remain beta/dev-only until their contracts are intentionally frozen.
  Date/Author: 2026-04-27 / Codex
...
- `scenery version --json` returned `scenery.version.v1`.
- `scenery inspect docs --json --repo-root <scenery-repo-root>` passed with `missing_count=0`, `review_due_count=0`, and `stale_count=0`.
- `scenery harness self --json --write` passed and wrote `.scenery/harness/self-latest.json`.
...
The CLI dispatcher lives in `cmd/scenery/main.go`. The stable commands to freeze for v0 are expected to be `scenery run`, `scenery build`, `scenery check --json`, `scenery inspect ... --json`, `scenery logs --jsonl`, `scenery test`, and `scenery gen client`. `scenery dev` is the development-platform command after the command split.

The current development supervisor lives in `cmd/scenery/dev_supervisor.go`. It owns dashboard, local proxy, removed agent transport/dashboard endpoints, app child process lifecycle, file watching integration, process output capture, and dashboard state.

The file watcher lives in `cmd/scenery/watch.go`. It has historically watched only selected files such as `.scenery.json`, `.go`, `.cpp`, and `.h`, which may miss build-affecting files like `go.mod`, `go.sum`, `.env`, and `.env.local`.
...
The dashboard UI source lives in `ui/` and is embedded by the CLI through `scenery.sh/ui`. A clean release build must either include built `ui/dist` assets, generate them in the release process, or avoid requiring them for headless/stable builds.

Terms used in this plan:

- Stable v0 means the supported behavior that users and agents can rely on without beta labels.
- Dev-only means a feature is useful in `scenery dev` but not part of the production-like runtime contract.
...
Milestone 2 completes the runtime/dev boundary. At the end of this milestone, `scenery dev` owns the development platform and headless `scenery run` starts only the app runtime. This milestone is complete when the acceptance criteria in [0001-devrun-command-split.md](0001-devrun-command-split.md) are satisfied.

Milestone 3 removes release build blockers. At the end of this milestone, a clean checkout can run `go install ./cmd/scenery` without missing embedded assets. Release docs state the required Go version and the Bun/UI build expectations. Release packaging excludes `.DS_Store`, `__MACOSX`, caches, and other local artifacts.

Milestone 4 hardens public-router safety. At the end of this milestone, pprof, `/__scenery/config`, `/__scenery/pubsub/clear`, platform stats, dashboard reporting, and other dev/admin endpoints are not mounted on the public app listener by default. If any remain, they are explicitly gated, documented, tested, and safe for local-only use.
...
Milestone 6 makes build artifacts safe and deterministic. At the end of this milestone, build workspace copying includes only files needed to compile and run the app. `.env`, `.env.*`, `.git`, `.scenery` runtime state, `node_modules`, editor files, caches, and local artifacts are excluded unless explicitly required and documented.

Milestone 7 fixes framework semantics before freeze. At the end of this milestone, response encoding honors expected Go JSON behavior for `json:"-"`, `omitempty`, embedded structs, pointers, headers, `scenery:"httpstatus"`, and custom marshalers, or documents any deliberate custom behavior with tests.
...
Start by updating the release contract before changing behavior. The repo should have one canonical local contract that says what is stable, what is beta, and what is dev-only. Use `docs/local-contract.md` as the canonical document, and keep `cmd/scenery/main.go` usage text aligned with it.

Next, finish the command split work tracked by [0001-devrun-command-split.md](0001-devrun-command-split.md). Do not make other release-hardening work depend on a dev supervisor hidden inside `scenery run`.

Then handle clean-checkout reproducibility. Verify whether `ui/dist` and other embedded assets are required for `go install ./cmd/scenery`. If they are required, choose one explicit release strategy: commit built assets, generate them in release packaging, or move embedding behind a development build boundary. The strategy must be documented and validated from a clean checkout.
...
Then make local HTTPS proxy and trust installation opt-in. `scenery dev` may support `--proxy` and a separate explicit trust flag. Do not surprise users by mutating system trust stores. `scenery run` should never install trust roots.
...
Then lock down build workspace copying. Replace broad file copying with an allowlist plus explicit asset inclusion behavior. Add tests that prove `.env`, `.env.local`, `.git`, `.scenery`, `node_modules`, `.DS_Store`, and `__MACOSX` are excluded.
...
List current command and runtime boundary references:
    ...
    rg -n "case \"run\"|case \"dev\"|runCommand|devCommand|runWithWatch|newDevSupervisor|runtimeapp|localproxy|pprof|platform.Stats|pubsub/clear|__scenery/config" cmd internal runtime
...
Audit public runtime routes:
    ...
    rg -n "__scenery/config|pubsub/clear|platform.Stats|debug/pprof|Access-Control-Allow-Origin|Access-Control-Allow-Credentials" runtime cmd internal
...
Audit secrets loaders:
    ...
    rg -n "LoadDotEnv|\\.env|secrets|DatabaseURL|SCENERY_ENV|SCENERY_MODE" cmd internal runtime

Add or update tests for the release blockers:

    go test ./cmd/scenery ./internal/build ./runtime

Run the full validation gate:

    scripts/release-gate.sh

The script also runs `go install ./cmd/scenery`, `scenery harness self --json --write`, dashboard UI builds, a clean source-copy install, fixture smoke, optional external app smoke, public-router safety checks, production secrets checks, and artifact hygiene checks. Set `SCENERY_RELEASE_GATE_EXTERNAL_APP_ROOT` to include a read-only external scenery app in the gate.
...
- A clean checkout can run `go install ./cmd/scenery`.
- `go test ./...` passes.
...
- `scenery inspect docs --json --repo-root <scenery-repo-root>` reports no missing or stale documents.
- CLI usage text and `docs/local-contract.md` describe the same commands.
- `scenery version --json` exists or the lack of version command is explicitly deferred before release.
- Stable, beta, dev-only, and compatibility-mode features are labeled in docs.
- `scenery run` is headless and production-like.
- `scenery dev` owns dashboard, local HTTPS proxy, frontend proxy, removed agent transport, file watching, and development-only UI.
...
- `.env`, `.env.local`, `.git`, `.scenery` runtime state, `node_modules`, `.DS_Store`, and `__MACOSX` are not copied into build workspaces or release archives.
- Response encoding tests cover `json:"-"`, `omitempty`, embedded structs, pointer fields, header fields, `scenery:"httpstatus"`, and custom marshalers.
...
Do not delete development functionality while moving it behind `scenery dev`. The recovery path for a broken command split is to keep `scenery dev` on the existing supervisor path and continue narrowing `scenery run` separately.
...
Release-stable generated artifacts:
    ...
    .scenery/gen/app.json
    .scenery/gen/routes.json
    .scenery/gen/services.json
    .scenery/gen/manifest.json
...
No new external dependency is expected for this plan. Prefer the Go standard library and existing scenery packages.

Expected CLI interfaces to freeze:

    scenery dev [development flags]
    scenery run [--port <n>] [--listen <addr>] [--app-root <path>] [--env <name>] [--log-format text|json]
    scenery build [--app-root <path>] [-o <path>] []
    scenery check --json [--app-root <path>]
    scenery inspect app|routes|services|endpoints|wire|build|paths|traces|metrics|docs --json
    scenery logs --jsonl [--app-root <path>]
    scenery test [--app-root <path>] [go test flags/packages...]
    scenery gen client [<app-id>] --lang typescript --output <path> [--app-root <path>]
