...
* 2026-05-27: Restarting the agent without `--router-tls` now reports `router_scheme=https`, and a fresh ONLV session emits HTTPS routes for API, dashboard, MCP, frontends, Electric, Grafana, and Temporal. TLS route smokes used `curl -k` so the validation covered the router/certificate path without depending on host trust-store state.
...
* `onlava dev` can start or continue a usable ONLV session with agent dashboard, API, frontend, MCP, and Electric routes.
