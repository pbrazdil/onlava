# scenery Data Platform

This ExecPlan is a living document. Update Progress, Surprises & Discoveries, Decision Log, and Outcomes & Retrospective as work proceeds.

## Purpose / Big Picture

scenery should gain a native Go data platform for dynamic CRM-style objects without copying Twenty line by line. The useful idea to preserve is metadata-defined objects and fields backed by real PostgreSQL tables and columns. The implementation must be scenery-native, Go-first, migration-aware, and correct for local live updates.

The first implementation is a narrow vertical slice:

```text
ordinary scenery app service
        |
        v
scenery.sh/data
        |
...
PostgreSQL metadata + real record tables + transactional outbox
        |
        v
raw scenery SSE endpoint for query-aware live updates
```

When this plan is complete, a fixture app under `testdata/apps/data-platform` can start as a normal scenery app, create a dynamic object such as `company`, add fields such as `name`, `stage`, and `arr`, insert/query/update/delete records, and receive an SSE event only for matching query subscriptions. The feature is not a dynamic ORM, not GraphQL, not a Twenty UI port, and not an external broker integration.
...
- [x] (2026-05-08 20:08Z) Defined `scenery.sh/data` as the small app-facing facade and `internal/objectstore` as the implementation boundary.
...
- [x] (2026-05-08 20:08Z) Added `testdata/apps/data-platform` fixture endpoints and unit plus `SCENERY_TEST_DATABASE_URL`-gated PostgreSQL integration coverage.
- [x] (2026-05-08 19:54Z) Ran validation: `go test ./...`, `go install ./cmd/scenery`, `scenery check --app-root testdata/apps/data-platform --json`, and `scenery harness self --json --write`.

## Surprises & Discoveries

- `pgxpool/pgxpool.go` already wraps `github.com/jackc/pgx/v5/pgxpool` with scenery DB query tracing. The data platform should accept a pool or transaction from app code and benefit from this existing tracing rather than discover database URLs itself.
...
- There is no repository-wide PostgreSQL integration harness discovered by search. Existing DB-related tests are mostly URL/config/unit tests. The first PostgreSQL integration tests should be skipped unless `SCENERY_TEST_DATABASE_URL` is set, and the fixture instructions should document that.
...
- `scenery check --app-root testdata/apps/data-platform --json` passes with ordinary scenery typed endpoints and a raw auth SSE endpoint, so the first slice did not require parser, codegen, or runtime route changes.
- `go test ./...` passes without `SCENERY_TEST_DATABASE_URL`; the PostgreSQL vertical-slice integration test skips clearly when that variable is unset.

## Decision Log

- Decision: Put the core implementation in `internal/objectstore` and expose a deliberately small public package at `scenery.sh/data`.
  Rationale: User apps need ordinary scenery Go APIs and fixture endpoint types, but parser/build/codegen should not learn dynamic CRM metadata. Keeping implementation internal preserves room to change storage and query internals.
  Date/Author: 2026-05-08 / Codex

- Decision: Do not add new `//scenery:` directives in the first vertical slice.
  Rationale: Dynamic object metadata lives in PostgreSQL at runtime, while `internal/model` is the parsed app model for source-level scenery semantics. Ordinary services and raw endpoints are enough to prove the vertical slice.
  Date/Author: 2026-05-08 / Codex
...
- Decision: Use SSE for the first live-update transport.
  Rationale: SSE fits the single-server local runtime, requires only `net/http`, supports reconnect with `Last-Event-ID` or an `after_seq` query parameter, and is already supported by raw scenery endpoint streaming.
  Date/Author: 2026-05-08 / Codex
...
- Decision: Use `scenery_data` for metadata/outbox tables and `scenery_data_records` for physical dynamic object tables in the first implementation.
  Rationale: Dedicated scenery-owned schemas avoid collisions with user app tables while still creating real PostgreSQL tables and columns. The exact table-name derivation must be deterministic, validated, and recorded in object metadata.
  Date/Author: 2026-05-08 / Codex
...
Implemented `scenery.sh/data` and `internal/objectstore` with PostgreSQL metadata bootstrap, real object tables, real field columns, conservative schema migrations with advisory locks and verification, metadata-validated SQL query compilation, transactional record mutations, outbox events, in-process query-aware live routing, and SSE replay/fanout.

Added `testdata/apps/data-platform`, which exposes ordinary scenery services for object/field/record APIs and a raw auth SSE endpoint. `scenery check --app-root testdata/apps/data-platform --json` passes, proving no new directives or runtime server model were needed.

Validation passed with `go test ./...`, `go install ./cmd/scenery`, and `scenery harness self --json --write`. The harness still reports the pre-existing large-file warnings for `internal/clientgen/typescript_render.go` and `internal/codegen/generator.go`. PostgreSQL integration coverage is present but skipped unless `SCENERY_TEST_DATABASE_URL` is set; this environment variable was not set during the final validation run, so DB-backed execution remains the one manual follow-up on this machine.

## Context and Orientation

The scenery repository is a Go module at `scenery.sh`. User apps depend on public packages such as `scenery.sh`, `auth`, `errs`, `middleware`, `pubsub`, `cron`, and `pgxpool`, and declare services with `//scenery:` directives.

Important architecture boundaries from `ARCHITECTURE.md`:

- `cmd/scenery` is CLI orchestration only. Non-CLI packages must not import it.
...
- `testdata/apps`: fixture apps use `.scenery.json`, `go.mod`, `replace scenery.sh => ../../..`, and ordinary scenery service packages.

Terminology:

- A data tenant is the isolation unit for dynamic object metadata and records. Do not call it a workspace in new code because scenery already uses build workspace terminology.
...
Milestone 2 creates metadata and physical schema. This is complete when opening or initializing the store creates `scenery_data` metadata tables, creates `scenery_data_records`, can create a data tenant, can create an object with a physical table, can create fields with physical columns, records every DDL operation in `scenery_data.schema_migrations`, uses PostgreSQL advisory locks, verifies the resulting schema, and bumps `schema_version` only after verification.
...
Milestone 6 adds the scenery fixture app and integration tests. This is complete when `testdata/apps/data-platform` exposes ordinary `//scenery:api` endpoints for object/field/record CRUD/query and a raw SSE endpoint, and tests prove the expected vertical slice against PostgreSQL when `SCENERY_TEST_DATABASE_URL` is set.
...
For mutations, require an explicit `Actor` argument. A helper may convert current scenery auth state into an actor for convenience, but core mutation methods should not rely on globals. Permission checks go through an interface with an allow-all default:
...
For live updates, first support one process and one database. The outbox gives durable sequence and replay. The in-process router handles active subscriptions only for the running app server. When a client reconnects with `after_seq`, read missed events from `scenery_data.outbox_events`, then continue with live fanout. Do not add Redis, NATS, Kafka, gqlgen, an ORM, or a migration framework.

Add the fixture only after the package can perform object, field, record, and event operations in focused unit tests. The fixture app should demonstrate that this is native scenery code:

```go
//scenery:api auth path=/data/objects method=POST
func CreateObject(ctx context.Context, req data.CreateObjectRequest) (*data.Object, error)

//scenery:api auth path=/data/objects/:object/fields method=POST
func CreateField(ctx context.Context, object string, req data.CreateFieldRequest) (*data.Field, error)

//scenery:api auth path=/data/objects/:object/records/query method=POST
func QueryRecords(ctx context.Context, object string, req data.QueryRecordsRequest) (*data.RecordPage, error)

//scenery:api auth path=/data/objects/:object/records method=POST
func CreateRecord(ctx context.Context, object string, req data.CreateRecordRequest) (*data.RecordResponse, error)

//scenery:api auth path=/data/objects/:object/records/:id method=PATCH
func UpdateRecord(ctx context.Context, object string, id string, req data.UpdateRecordRequest) (*data.RecordResponse, error)

//scenery:api auth path=/data/objects/:object/records/:id method=DELETE
func DeleteRecord(ctx context.Context, object string, id string) (*data.DeleteRecordResponse, error)

//scenery:api auth raw path=/data/events method=GET
func Events(w http.ResponseWriter, r *http.Request)
```

If adding `scenery inspect data --json` becomes necessary, keep it narrow and stable. Update `docs/schemas`, inspect tests, and `docs/local-contract.md` in the same change. The first slice should prefer fixture endpoints and tests over new CLI surface.
...
12. Add `testdata/apps/data-platform` with `.scenery.json`, `go.mod`, service code, auth handler, and fixture tests or CLI integration helpers.
13. Add skipped PostgreSQL integration tests controlled by `SCENERY_TEST_DATABASE_URL`. Tests should create isolated temporary database schemas or unique data tenant keys and clean up after themselves.
...
```sh
SCENERY_TEST_DATABASE_URL='postgres://...' go test ./internal/objectstore ./testdata/...
```
...
If PostgreSQL is not available, the integration tests must skip with a clear message naming `SCENERY_TEST_DATABASE_URL`, and the final handoff must say which DB-backed checks remain manual.
...
- `scenery.sh/data` remains small and app-facing.
...
- Existing scenery fixture apps still build and tests pass.

## Idempotence and Recovery

Metadata bootstrap must be safe to call repeatedly. `create schema if not exists` and `create table if not exists` are acceptable for bootstrap, but object and field migrations must be represented in `scenery_data.schema_migrations` and verified.
...
Outbox delivery is at least once. SSE clients should deduplicate by `seq` or event id. Publishing must happen after commit. If in-process notification is missed, polling/replay from `scenery_data.outbox_events` must still deliver committed events.
...
```text
scenery_data.tenants
scenery_data.objects
scenery_data.fields
scenery_data.field_options
scenery_data.schema_migrations
...
- Trigger-backed outbox should later attach to physical record tables and write the same `scenery_data.outbox_events` shape. It should be designed after explicit mutation-layer behavior is stable.
...
The internal package may depend on existing repository dependencies, especially `github.com/jackc/pgx/v5` and `scenery.sh/pgxpool` compatible interfaces. It must not add Redis, NATS, Kafka, gqlgen, an ORM, or a migration framework.
...
No CLI command is required in the first slice. If `scenery inspect data --json` is added, it must include a schema under `docs/schemas`, tests in `cmd/scenery` or `internal/inspect`, and a local-contract update in the same implementation change.
