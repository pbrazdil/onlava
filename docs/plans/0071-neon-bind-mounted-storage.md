...
Scenery's self-hosted Neon dev cell currently stores much of its durable runtime
state in Docker-managed anonymous volumes. The Scenery substrate root is easy to
find at `~/.scenery/agent/substrates/neon`, but the actual MinIO, pageserver,
...
This plan moves Neon durable storage to Scenery-owned bind-mounted directories
...
storage substrate per Scenery agent home. New Git worktrees do not receive
separate storage directories. A worktree gets only its local branch pin at
`<worktree>/.scenery/worktree-db.json`; Neon branch isolation lives in shared
...
scenery worktree create pricing-agent --json
cd ../onlv-pricing-agent
scenery up
```

All worktrees share `~/.scenery/agent/substrates/neon/data`, while each worktree
...
- [x] 2026-06-09: Added start-time fresh-start guard behavior for existing Scenery Neon containers with Docker-managed `/data` volumes.
- [x] 2026-06-09: Updated status JSON/schema, uninstall semantics, docs, and tests for the explicit shared storage root.
- [x] 2026-06-09: Validated with focused Neon/worktree tests, `go test ./cmd/scenery`, `go test ./...`, docs inspection, JSON parsing, and diff whitespace checks.

## Surprises & Discoveries

- 2026-06-09: `scenery inspect docs --json` reports `summary.review_due_count: 1`; the only review-due document is `docs/ui-agent-contract.md`, unrelated to this storage plan.
- 2026-06-09: `scenery db neon status --json` on the current machine reports the active Neon root as `/Users/petrbrazdil/.scenery/agent/substrates/neon`, with `backend.json`, `branches.json`, `restore-points.json`, generated Compose, pageserver config, and compute templates under that root.
- 2026-06-09: `docker inspect` on the live `scenery-neon-*` containers shows anonymous Docker volumes mounted at `/data` for MinIO, pageserver, safekeepers, and storage broker. The pageserver also bind-mounts `pageserver_config` to `/data/.neon`, and branch compute containers only bind-mount generated compute templates.

## Decision Log

- Decision: Store Neon runtime data under `~/.scenery/agent/substrates/neon/data`, not under app roots or worktree roots.
...
- Decision: `scenery db neon uninstall` should preserve `data/` by default, and `scenery db neon uninstall --destroy-data` should remove it.
...
- Decision: New worktrees should only create or reuse `.scenery/worktree-db.json` pins and should not allocate storage directories.
...
the shared storage root/data directories through `scenery.db.neon.status.v1`.
`scenery db neon install --json` creates the storage directories under the agent
home, and generated Compose bind-mounts each durable `/data` path from
`./data/...`.

`scenery db neon start --json` fails closed when old Scenery Neon containers still
use Docker-managed `/data` volumes. The user-visible recovery path is fresh
destroy/reinstall, not migration. `scenery db neon uninstall --json` now
...
- `cmd/scenery/db_neon.go` generates the Neon cell state, writes `cell.json`, writes `compose.generated.yml`, starts/stops/restarts/uninstalls the generated Compose project, and reports `scenery.db.neon.status.v1`.
- `cmd/scenery/db_neon_runtime.go` probes Docker images, containers, and loopback listeners for `scenery db neon status --json`.
- `cmd/scenery/db_neon_state.go` owns local Neon state file read/write helpers.
- `cmd/scenery/db_neon_pin.go`, `cmd/scenery/db_neon_provider.go`, `cmd/scenery/db_neon_restore_points.go`, and `cmd/scenery/worktree.go` own pins, branch leases, restore points, and worktree behavior.
...
- `docs/schemas/scenery.db.neon.status.v1.schema.json` may need a storage section if status begins exposing bind-mounted data directories as machine-readable fields.
...
```text
/Users/petrbrazdil/.scenery/agent/substrates/neon
```
...
Add storage path helpers for the Neon substrate root. `scenery db neon install`
...
Compose must not rely on anonymous volumes for Scenery Neon services.

Milestone 3: Detect existing anonymous-volume cells and fail closed.

Before starting a bind-mounted cell, detect existing `scenery-neon-*` containers
...
`scenery db neon uninstall` should stop/remove containers and generated runtime
...
Scenery-owned containers. Cleanup must remain scoped to Scenery-owned Neon
containers and files.

Milestone 5: Preserve worktree branch behavior.

`scenery worktree create`, `scenery up`, `scenery db branch checkout`, and session
...
Next update Compose generation in `cmd/scenery/db_neon.go`. The generated file
...
`scenery db neon uninstall --destroy-data --json` and then reinstalling the dev
...
`scenery.db.neon.status.v1` or through generated file/status entries if that is
the narrower local pattern. `uninstall --destroy-data` must remove the bind data
root and can also remove Scenery-owned legacy containers/volumes as a destructive
...
1. Inspect current generated Compose and state helpers:
   ...
   ```sh
   rg -n "compose.generated|defaultNeonCellState|writeNeonCellState|uninstall|destroy-data|docker inspect|Mounts" cmd/scenery internal/neonselfhost
   ```
...
4. Add tests around generated storage:
   ...
   - `scenery db neon install --json` creates every `data/*` directory.
...
5. Add fresh-start guard behavior:
   ...
   - Detect legacy anonymous `/data` volumes from existing Scenery-labeled containers.
...
6. Update uninstall and destroy behavior:
   ...
   - Keep cleanup scoped to Scenery-owned Neon containers and generated state.
...
8. Update docs and schemas:
   ...
   - `SKILL.md`
   - `README.md`
   - `docs/schemas/scenery.db.neon.status.v1.schema.json` if status gains a storage object.
...
```sh
go test ./cmd/scenery -run 'TestDBNeon|TestWorktree|TestParseDBNeonArgs'
...
```sh
tmp="$(mktemp -d)"
go build -o "$tmp/scenery" ./cmd/scenery
"$tmp/scenery" db neon install --json
"$tmp/scenery" db neon status --json
```
...
- Fresh `scenery db neon install --json` creates `data/minio`, `data/pageserver`,
...
- A fresh start does not create anonymous Docker volumes for Scenery Neon
  durable storage.
...
Install and Compose generation must be idempotent. Re-running `scenery db neon
...
able to run `scenery db neon status --json`, inspect `compose.generated.yml`, and
rerun `scenery db neon start --json` after fixing Docker or port readiness.
...
```text
<worktree>/.scenery/worktree-db.json
```
...
```sh
go test ./cmd/scenery -run 'TestDBNeon|TestWorktreeCreateListAndRemoveWithNeonPin'
go test ./internal/neonselfhost
go test ./cmd/scenery
go test ./...
jq empty docs/knowledge.json docs/schemas/scenery.db.neon.status.v1.schema.json
scenery inspect docs --json
git diff --check
```
...
`scenery db neon uninstall --destroy-data` against that cell.
...
- `scenery db neon uninstall [--destroy-data] --json`
- `scenery worktree create|list|remove --json`
- `scenery db branch status|checkout|list|reset|restore|delete|diff --json`

Machine-readable contracts that may change:

- `scenery.db.neon.status.v1`
- `scenery.db.neon.cell.v1`
- Generated `compose.generated.yml`
- Generated state paths under `~/.scenery/agent/substrates/neon`

External dependencies:

- Docker and Docker Compose for the local Neon storage cell.
- Existing Neon images from `scenery.toolchain.json`.
