...
- [x] 2026-05-29: Investigated root integration next. Individual measurements showed `TestOnlavaDevReloadsOnGoChanges` at 9.420s, `TestOnlavaDevDashboardNotificationsAndremoved agent transport` at 10.380s, `TestOnlavaBuildProducesRunnableBinary` at 4.730s, and `TestOnlavaRunBasicApp` at 5.110s. These are real app/dev/build startup costs, not only process-slot queueing.
...
- [x] 2026-05-29: Removed the process-wide test workspace-key dependency from the real generated-workspace `onlava test` command-path test. It now uses a stable persistent app root and can run as a parallel test while still invoking a real generated-workspace `go test`.
...
- [x] 2026-05-29: Removed the dedicated `internal/harnessbrowser` package and moved the browser UI harness onto a small package-local CDP runner in `cmd/onlava`. The opt-in real-browser smoke passed against a real browser, and `go mod tidy` removed the stale chromedp dependency graph.
...
- [x] 2026-06-01: Removed the remaining scheduler caps from the default test paths by dropping root integration child `GOMAXPROCS=2`, the root integration process-slot limiter and `ONLAVA_INTEGRATION_PROCESS_SLOTS` override, and the self-harness `GOMAXPROCS`, `-p`, and `-parallel` settings.
- [x] 2026-06-01: Restored fingerprint-specific workspace binary names as default build behavior after removing the test workspace-key env seam, moved selected root integration CLI prebuild work into `TestMain`, and kept the full-stack dashboard/removed agent transport/proxy/reload test out of the parallel root integration pileup. The timing table now reports `TestOnlavaDevDashboardNotificationsAndremoved agent transport` around 1.5s instead of 6s.
...
- 2026-06-01: Collapsing workspace binaries back to a single `onlava-app` path made dev reload builds overwrite the currently running app binary. The dashboard/removed agent transport integration test then spent most of its time waiting on reload recovery. The same test could also be charged for the shared one-time CLI build when the installed `onlava` binary was stale.
...
- Decision: Keep generated app workspace binaries fingerprint-specific by default, without any test-only workspace key env.
  Rationale: Hot reload must compile the replacement binary beside the currently running binary, not overwrite a fixed `onlava-app` path. A source fingerprint in the binary name preserves reuse for unchanged builds and keeps reload behavior correct after removing the env seam.
