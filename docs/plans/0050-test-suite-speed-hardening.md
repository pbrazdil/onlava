...
- 2026-06-01: Collapsing workspace binaries back to a single `onlava-app` path made dev reload builds overwrite the currently running app binary. The dashboard/MCP integration test then spent most of its time waiting on reload recovery. The same test could also be charged for the shared one-time CLI build when the installed `onlava` binary was stale.
...
- Decision: Run the dashboard/MCP/proxy/reload integration test serially and prebuild the shared CLI binary in `TestMain` when selected root integration tests need it.
