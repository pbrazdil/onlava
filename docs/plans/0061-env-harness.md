...
Stop accidental environment-variable growth in onlava. AI agents and contributors have been adding new `ONLAVA_*`, tool, and app-facing environment names as a convenient escape hatch. That makes behavior harder to reason about, harder to test, and harder for target apps to operate. The intended policy is the opposite: if a setting is real product configuration, it belongs in `.onlava.json`, an explicit CLI flag, or a checked-in manifest. Environment variables should exist only for secrets, process identity injected by onlava, test-only controls, explicitly requested escape hatches, and compatibility names that are deliberately being retired.

After this work, `onlava harness self --json --write` must make ENV drift visible and, by default, fail when production code introduces a new unapproved environment variable. Adding a new environment variable should require an explicit registry entry, a documented rationale, tests, and a Decision Log entry in the relevant ExecPlan. The easiest path for future agents should be to avoid ENV variables entirely.
...
- [x] 2026-06-01: Full validation passed: focused env/harness tests, `go test ./cmd/onlava`, `go test ./...`, `go install ./cmd/onlava`, `onlava inspect docs --json`, `onlava harness self --json --write`, and `git diff --check`.

## Surprises & Discoveries

- 2026-06-01: `cmd/onlava/harness_drift.go` already has a first-pass environment scanner. It scans Go files for `ONLAVA_` tokens, skips architecture-generated/cache dirs, classifies `_test.go` and `testdata/` as test scope, and warns when runtime variables are not documented in `docs/environment.md`, `docs/local-contract.md`, `docs/grafana.md`, or `SKILL.md`. The next step is to make this registry-backed and fail-closed for new runtime variables, not merely warning-based.
- 2026-06-01: The same file currently records live `ONLAVA_*` values in the toolchain preflight report through `sortedOnlavaEnv(os.Environ())`. This is useful for local diagnostics but should be reviewed for redaction and separated from source-contract enforcement.
...
- Decision: Treat CLI flags, `.onlava.json`, and `onlava.toolchain.json` as the preferred homes for configurable behavior.
...
- Decision: Keep test-only env variables allowed but make the prefix and scope explicit.
  Rationale: Tests sometimes need process-level controls. Those names should be visibly separate from runtime contract names, preferably `ONLAVA_TEST_*` or package-local test helpers, and should not appear in app-facing docs as supported knobs.
  Date/Author: 2026-06-01 / OpenAI assistant

- Decision: Keep the registry under `docs/environment.registry.json` and validate it with `docs/schemas/onlava.environment.registry.v1.schema.json`.
  Rationale: This keeps the human env reference and machine-readable contract side by side, discoverable through `docs/knowledge.json`, and enforceable by `onlava harness self`.
  Date/Author: 2026-06-01 / OpenAI assistant
...
- `docs/schemas/onlava.environment.registry.v1.schema.json` plus self-harness schema validation for the registry.
...
- Registry-backed `onlava harness self` drift checks that fail on unregistered production env usage, test-only env usage in production code, undocumented registered runtime env usage, and direct production `os.Getenv`/`os.LookupEnv`/`os.Environ`/`os.Setenv`/`os.Unsetenv` outside `internal/envpolicy`.
...
- `onlava inspect docs --json` passed with 25 documents and zero missing or stale docs.
- `onlava harness self --json --write` passed. Relevant summaries: contract drift checks `diagnostics=0`, env vars `168`; Go tests `packages=43`; schema validation `errors=0`, validated `12`.
...
```text
cmd/onlava/harness_drift.go
cmd/onlava/harness_self.go
cmd/onlava/harness_schema.go
cmd/onlava/harness_*_test.go
docs/environment.md
...
- `onlava harness self --json --write` already runs contract drift checks and emits a `onlava.harness.drift.v1` report.
- `buildHarnessEnvVarReport` scans Go source for `ONLAVA_` tokens and records whether each runtime variable is documented.
- Test-only variables are recognized when found in `_test.go`, `testdata/`, `ONLAVA_TEST_*`, or `ONLAVA_INTEGRATION_*` contexts.
...
- **Injected variable** means a name set by onlava for child processes, frontends, dashboards, workers, or managed sidecars.
...
- Existing `ONLAVA_*`, standard external names such as `DATABASE_URL`, `TEMPORAL_ADDRESS`, `OTEL_EXPORTER_*`, and app-defined auth env examples are accounted for rather than ignored because they are not prefixed with `ONLAVA_`.

Implementation notes:

- Extend `extractOnlavaEnvTokens` into a more general token scanner, but avoid parsing secrets or values. The harness only needs names and file references.
...
```json
{
  ...
  "owner": "onlava dev platform",
  "rationale": "Disable or require dev Grafana while the managed sidecar is optional.",
  "preferred_surface": ".onlava.json dev observability config when promoted beyond dev-only behavior",
  "replacement": "",
  "sunset": "",
...
- The registry can represent prefix families such as `ONLAVA_FRONTEND_<NAME>_ADDR`, `ONLAVA_VICTORIA_METRICS_*`, `GF_*`, or `OTEL_EXPORTER_*` without forcing an infinite list of generated concrete names.
...
Make `onlava harness self` fail when a production env name appears outside the registry or violates its allowed scope.
...
- The drift report identifies file paths and suggested actions: remove the env, move the configuration to `.onlava.json` or a CLI flag, or add a registry entry with rationale if explicitly approved.
...
Audit current user-input env variables and decide which should become `.onlava.json`, CLI flags, or managed manifest fields.

High-value candidates:

- Tool paths and downloads should prefer `onlava.toolchain.json`, `ONLAVA_TOOLCHAIN_DIR`, and explicit per-tool override only when truly necessary.
- Grafana/Victoria ports, reuse, versions, downloads, and public URLs should be evaluated for `.onlava.json dev.observability` or managed toolchain fields.
...
- New configurable behavior added during this work uses `.onlava.json`, CLI flags, or manifests unless this plan’s Decision Log records why env is required.

Current audit:

| Class | Decision | Notes |
| --- | --- | --- |
| App identity and routing injection | keep | Injected variables such as `ONLAVA_APP_ID`, `ONLAVA_LISTEN_ADDR`, session IDs, routed API/Electric URLs, and Temporal task-queue/build metadata are process identity, not user configuration. |
| Secrets and service URLs | keep | Secret or credential-bearing variables such as `DATABASE_URL`, `ONLAVA_AUTH_JWT_SECRET`, and `TEMPORAL_API_KEY` stay env-backed and are marked secret for harness redaction. |
...
Start by extending the existing harness drift code rather than adding a second checker. The current code already participates in `onlava harness self`, emits JSON, and has the right diagnostic shape. Change it from "find undocumented ONLAVA tokens" to "compare discovered env names against a registry and fail on policy violations."
...
1. Add focused tests around the existing scanner in `cmd/onlava/harness_drift_test.go` if they do not already exist. Cover production source, test source, docs-only mentions, prefix families, and non-`ONLAVA_` names such as `DATABASE_URL` and `TEMPORAL_ADDRESS`.
...
7. Flip production unregistered variables to errors and add a fixture/self-harness test that injects a fake `ONLAVA_NEW_ESCAPE_HATCH` usage into a temporary repo and expects failure.
...
```sh
go test ./cmd/onlava -run 'TestHarness.*Env|TestEnvPolicy|TestHarnessSelf'
go test ./cmd/onlava
go test ./...
...
```sh
# In a temporary copy/fixture, add a production reference to ONLAVA_FAKE_NEW_ENV.
onlava harness self --repo-root <fixture> --json
# Expected: ok false, diagnostic names ONLAVA_FAKE_NEW_ENV and suggests removing it or adding a registry entry with rationale.

# In a temporary copy/fixture, add ONLAVA_TEST_ONLY_EXAMPLE in production code.
onlava harness self --repo-root <fixture> --json
# Expected: ok false, diagnostic says test-only env used by production code.

# Set a secret-like env while running self-harness.
ONLAVA_AUTH_JWT_SECRET=example onlava harness self --json
...
- Docs make `.onlava.json`, CLI flags, and manifests the default answer for configurability.
...
cmd/onlava/harness_drift.go
cmd/onlava/harness_drift_test.go
cmd/onlava/harness_schema.go
...
Avoid adding generated snapshots by hand. Regenerate harness outputs with `onlava harness self --json --write` when the implementation changes the snapshot contract.

## Interfaces and Dependencies

This work touches onlava’s development and agent-facing contracts, but it should not add external dependencies. Use the Go standard library for scanning, JSON loading, sorting, and path handling.

Interfaces affected:

- `onlava harness self --json --write`: stricter drift diagnostics and possibly expanded `onlava.harness.drift.v1` env fields.
