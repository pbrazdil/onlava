# Pulse Agent-First Harness Plan

Source: [OpenAI Harness Engineering](https://openai.com/index/harness-engineering/), read on 2026-04-27.

This plan translates the article's agent-first engineering ideas into concrete Pulse work. It is intentionally repo-local because agent-visible knowledge must live in the repository, not in chat history.

## Operating Model

Pulse should optimize for agent legibility and fast feedback loops:
...
- Stable inspect surfaces: `pulse inspect app|routes|services|endpoints|wire|build|paths|traces|metrics|docs --json`.
- App harness: `pulse harness --json --write`.
- Repo self-harness: `pulse harness self --json --write`.
...
- Add `pulse harness ui --json [--repo-root <path>] [--app-root <path>] [--headed]`.
- Start or reuse a local Pulse app fixture.
...
- Capture screenshots into `.pulse/harness/ui/`.
...
- A broken dashboard route causes `pulse harness ui --json` to fail with a specific route and screenshot path.
- The command can run headless by default and headed for local debugging.
- `pulse harness self --json --write` can optionally include UI harness output later without making the fast path heavy.
...
- Store failure artifacts under `.pulse/harness/artifacts/`.
- Capture command output tails, screenshots, trace IDs, request/response transcripts, and repro commands where applicable.
- Add `pulse inspect harness --json` or extend `pulse inspect paths --json` to expose latest harness artifact locations.
...
- Validate representative command outputs for `pulse inspect ... --json`, `pulse check --json`, `pulse harness --json`, `pulse harness self --json`, `pulse logs --jsonl`, and `pulse admin ... --json`.
- Prefer a small internal validator for the subset of JSON Schema used by Pulse before adding a new dependency.
- If a dependency becomes necessary, document the concrete payoff in the architecture allowlist.

Acceptance:

- `pulse harness self` fails if a schema and command output drift.
...
- A forbidden package edge fails `pulse harness self` with a remediation.
...
- Add `pulse inspect debt --json` or extend `pulse inspect docs --json` with debt rollups.
...
- Add `pulse harness review --json` or document a standard command sequence.
...
Implement `pulse harness ui --json` as the next major harness capability. Keep the first version small:
