...
It is intentionally optimized for agents. Keep it short enough to scan, precise enough to execute, and biased toward machine-readable commands over prose.
...
3. `docs/agent-guide.md` explains how agents should use onlava, MCP, generated artifacts, and app-local instructions.
4. `docs/local-contract.md` is the contract for CLI grammar, JSON schemas, generated artifacts, and stability labels.
...
When implementation and docs disagree, treat it as documentation or implementation drift. Fix the drift in the same change when practical.
...
Do not revive deprecated non-onlava APIs, legacy directive spellings, or compatibility aliases unless an explicit active plan says to do so.
...
- Keep generated app model and machine-readable JSON contracts stable. If a JSON shape changes, update schemas, docs, tests, and harness expectations together.
- Do not commit machine-local state or generated cache output from `.onlava/`, Grafana, Victoria, node modules, coverage, `.DS_Store`, or secret files.
- Do not expose secrets in docs, logs, generated artifacts, MCP responses, or examples.
...
Agents should prefer MCP for interactive local app work when a dev session is already running, especially for:

- listing services/endpoints/auth/middleware/cron metadata
- calling endpoints through the local runtime
- reading recent traces and trace spans
- querying the discovered development PostgreSQL database
- reading selected app source files
...
Client apps should keep a small app-local `AGENTS.md` that imports the onlava skill conceptually and records only app-specific facts:

- app root and `.onlava.json` location
- frontend roots and generated client output paths
- required local env/secrets names without values
