...
const registryRoot = path.resolve(projectRoot, process.env.SCENERY_SHADCN_REGISTRY_ROOT ?? path.join("registry", "scenery"));
const shadcnVersion = process.env.SCENERY_SHADCN_VERSION ?? "shadcn@4.7.0";

if (args.length === 0) {
  console.error("usage: bun run shadcn:add @scenery/<item> [...]");
  process.exit(1);
}
...
if (!existsSync(path.join(projectRoot, "components.json"))) {
  console.error("Refusing to run shadcn outside the scenery UI package.");
...
for (const item of parsed.items) {
  ...
  if (!item.startsWith("@scenery/")) {
    console.error(`Refusing non-scenery shadcn item: ${item}`);
    console.error("Use @scenery/<item> from the approved scenery registry.");
    process.exit(1);
  }
  const registryName = item.slice("@scenery/".length);
  ...
  if (!existsSync(path.join(registryRoot, `${registryName}.json`))) {
    console.error(`Unknown @scenery registry item: ${item}`);
    console.error(`Expected ${path.relative(projectRoot, path.join(registryRoot, `${registryName}.json`))}`);
    process.exit(1);
  }
}

if (parsed.overwrite && process.env.SCENERY_SHADCN_OVERWRITE !== "1") {
  console.error("Refusing --overwrite unless SCENERY_SHADCN_OVERWRITE=1 is set.");
  process.exit(1);
}
...
function parseArgs(input) {
  ...
  if (items.length === 0) {
    return { error: "No @scenery registry item provided." };
  }
