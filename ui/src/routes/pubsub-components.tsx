...
export function queueID(topicName: string, subscriptionName: string) {
  return `${topicName}\u0000${subscriptionName}`;
}

export function findSubscription(topics: PubSubTopic[], id: string): PubSubSubscription | null {
...
export function TooltipRow({ y, color, label, value }: { y: number; color: string; label: string; value: string }) {
  return (
    <>
...
export function LegendItem({ x, color, label, value }: { x: number; color: string; label: string; value: string }) {
  return (
...
export function SubscriptionRow({ sub }: { sub: PubSubSubscription }) {
...
export function PubSubMessageRow({
  message,
  open,
  attempts,
  onToggle,
...
export function JSONPreview({ value, error, label }: { value: unknown; error?: string; label: string }) {
...
export function MiniStat({ label, value, tone }: { label: string; value: string; tone?: "live" | "error" }) {
  return (
    <div>
...
export function summarize(topics: PubSubTopic[]) {
  let subscriptions = 0;
  let pending = 0;
  let pickedUp = 0;
  let inFlight = 0;
...
export function mergeLiveSnapshot(current: PubSubSnapshot | null, next: PubSubSnapshot): PubSubSnapshot {
...
export function periodToMs(period: (typeof chartPeriods)[number]["value"]) {
  switch (period) {
    case "5m":
      return 5 * 60 * 1000;
    case "1h":
...
export function normalizeChartDomain(points: ChartPoint[], start: number, end: number) {
...
export function linePath<T>(items: T[], getX: (item: T) => number, getY: (item: T) => number) {
  return items.map((item, index) => `${index === 0 ? "M" : "L"} ${getX(item).toFixed(2)} ${getY(item).toFixed(2)}`).join(" ");
}

export function nearestPoint(points: ChartPoint[], targetTime: number): ChartPoint | null {
...
export function upsertMessage(current: PubSubMessage[], next: PubSubMessage) {
...
export function upsertAttempt(current: PubSubMessageAttempt[], next: PubSubMessageAttempt) {
...
export function messageToAttempt(message: PubSubMessage): PubSubMessageAttempt {
  return {
...
export function messageWithinFilters(
  message: PubSubMessage,
  period: (typeof chartPeriods)[number]["value"],
  topicFilter: string,
  queueFilter: string,
...
export function compareMessages(a: PubSubMessage, b: PubSubMessage) {
  return Date.parse(b.inserted_at || "") - Date.parse(a.inserted_at || "");
}

export function messageRowKey(message: PubSubMessage) {
...
export function formatTimestampValue(value?: string) {
...
export function formatWorkers(sub: PubSubSubscription) {
