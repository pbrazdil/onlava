...
func GenerateBindingsWithKnownActivities(appRoot, appName, outDir string, knownActivities []string) (BindingResult, error) {
	...
	if strings.TrimSpace(outDir) == "" {
		outDir = filepath.Join(appRoot, ".scenery", "workers", "generated")
	}
...
func generateManifestBindings(outDir string, manifest Manifest) ([]BindingFile, error) {
	...
	case "python":
		path := filepath.Join(dir, "scenery_worker.py")
	...
	case "typescript":
		path := filepath.Join(dir, "scenery_worker.ts")
		if err := writeTemplate(path, typescriptBindingTemplate, data); err != nil {
			return nil, err
		}
		return []BindingFile{{Path: filepath.ToSlash(path), Language: manifest.Language, Manifest: filepath.ToSlash(manifest.Path)}}, nil
	default:
		metadataPath := filepath.Join(dir, "scenery_worker.json")
...
var pythonBindingTemplate = template.Must(template.New("python-worker").Parse(`# Code generated by scenery worker bindings; DO NOT EDIT.
...
async def {{ .FuncName }}(payload):
    """Implement scenery activity {{ .Name }}.
...
var typescriptBindingTemplate = template.Must(template.New("typescript-worker").Parse(`// Code generated by scenery worker bindings; DO NOT EDIT.
...
export async function {{ .FuncName }}(payload: unknown): Promise<unknown> {
  throw new Error({{ printf "%q" (printf "Implement scenery activity %s" .Name) }});
}
{{ end }}
